...
- **API Gateway Routing**: Requests to `/api/events/*` and `/ical` are proxied to the Node.js Lambda function via a separate Terraform configuration (`infrastructure/events/terraform`).
- **DynamoDB Schema (`Events-dev`)**: 
  - PK: `eventId` (String, UUID)
  - Attributes: `title`, `date`, `category`, `capacity`, `currentRsvps`, `version` (Number).

### 11.2 RSVP Logic & Optimistic Locking
...
4. If successful, the user is added to the `RSVP_Table`.

### 11.3 Catalog UI (`EventsDashboard.svelte`)
...
## 12. The Velvet Rope (Tiered Gating)
...
### 12.1 Backend Middleware API Gating
...
### 12.2 Frontend State Management & UI
