# CMIS Platform — Team Handoff

For the next team or maintainer taking over the codebase (including contributions from Team Gig 'Em and Team 12th Man).

## What this repo contains

- **External Core (Team Gig 'Em):** external user auth (Cognito), roles (PARTNER / FORMER_STUDENT / FRIEND), graduation handover, graduate claim (magic link), and related APIs.
...
- **Backend:** 
  - Python 3.12 Lambda in `services/external-service/` (single handler, path-based routing for Auth/Handover).
  - Node.js Express Lambda in `services/event-service/` (Events CRUD, RSVP logic).
- **Frontend:** Svelte + Vite in `frontend/` (Login, Register, Profile, Handover, Handover History, Claim, Forgot/Reset password, Events Catalog, Velvet Rope UI).
