module "external_core" {
  ...
  project_name = var.external_services_project_name
  ...
  # Temporarily hard-disable mentorship schedule EventBridge resources to avoid CI
  # failures on terraform-user missing events:ListTagsForResource. Re-enable after
  # IAM permissions are granted and state is cleaned.
  enable_mentorship_annual_eventbridge = false
