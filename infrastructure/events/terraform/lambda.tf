# Deployment package: zip the events-service source code.
...
# Shared environment variables for the Event Service Lambda.
locals {
  lambda_env = {
    # DynamoDB Tables
    ...
    # Velvet Rope API Integration: Connects to Team Howdy's Admin service.
    ...
    REMINDER_NOTIFICATION_MODE = lower(var.reminder_notification_mode)
  }
}
...
# RSVP Stream Mapping: Automatically promotes waitlisted users via DynamoDB Streams.
