# Event Service Module (Team 12th Man)
# This module orchestrates the infrastructure for event management, RSVPs, and waitlists.
module "events_core" {
  source = "./events/terraform"

  aws_region                  = var.aws_region
  stage                       = var.environment
  cognito_user_pool_id        = aws_cognito_user_pool.cmis.id
  cognito_client_id           = aws_cognito_user_pool_client.cmis_spa.id
  lambda_role_arn             = aws_iam_role.lambda_role.arn
  domain_api_url              = "${module.admin_core.api_gateway_url}/domain"
  config_api_url              = "${module.admin_core.api_gateway_url}/config"
  ses_verified_sender         = var.ses_verified_sender
  reminder_notification_mode  = var.reminder_notification_mode
  student_profiles_table_name = module.student_core.student_profiles_table_name
  reminder_rule_prefix        = "events-core-reminder-${var.environment}"

  # Path to event-service source relative to this module directory
  events_service_path = "../../../services/event-service"
}
