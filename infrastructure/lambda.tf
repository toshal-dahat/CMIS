  ...
  policy = jsonencode({
    ...
    Statement = [
...

      {
        ...
        Resource = "arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:${var.external_services_project_name}-external-service"
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = "arn:aws:events:${var.aws_region}:${data.aws_caller_identity.current.account_id}:rule/events-core-reminder-*"
      }
    ]
  })
}
...
# SES verified sender identity for RSVP confirmation and waitlist-promotion emails
# (Team 12th Man — event-service DynamoDB Streams trigger). The address is hardcoded
# because SES sandbox accounts require each sender to be individually pre-verified; there
# is no API to verify an arbitrary address at deploy time. Extracting this to a Terraform
# variable or SSM parameter is tracked as Open Question #3 in TEAM_12TH_MAN_INVENTORY.md.
