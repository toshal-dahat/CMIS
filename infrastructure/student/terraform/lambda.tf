...
locals {
  reminder_scheduler_role_arn = trimspace(var.reminder_scheduler_role_arn) != "" ? trimspace(var.reminder_scheduler_role_arn) : var.lambda_role_arn
  ...
  lambda_env = {
    ...
    EVENTS_API_BASE_URL          = var.events_api_base_url
    SES_VERIFIED_SENDER          = var.ses_verified_sender
    FRONTEND_URL                 = var.frontend_url
    REMINDER_SCHEDULE_GROUP_NAME = var.event_reminder_schedule_group_name
  }
}
...
# --- Event CRUD proxy + reminder schedule manager ---
resource "aws_lambda_function" "event_reminders_api" {
  function_name    = "student-core-event-reminders-api-${var.stage}"
  role             = var.lambda_role_arn
  runtime          = var.lambda_node_runtime
  handler          = "src/handlers/eventReminders.eventCrudProxy"
  filename         = data.archive_file.lambda_zip.output_path
  source_code_hash = data.archive_file.lambda_zip.output_base64sha256

  environment {
    variables = merge(local.lambda_env, {
      REMINDER_TARGET_LAMBDA_ARN  = aws_lambda_function.event_reminder_dispatch.arn
      REMINDER_SCHEDULER_ROLE_ARN = local.reminder_scheduler_role_arn
    })
  }
...
# --- User reminder preference toggle ---
resource "aws_lambda_function" "event_reminder_preference" {
  function_name    = "student-core-event-reminder-preference-${var.stage}"
  role             = var.lambda_role_arn
  runtime          = var.lambda_node_runtime
  handler          = "src/handlers/eventReminders.reminderPreference"
  filename         = data.archive_file.lambda_zip.output_path
  source_code_hash = data.archive_file.lambda_zip.output_base64sha256
...
# --- Scheduled one-hour reminder dispatcher ---
resource "aws_lambda_function" "event_reminder_dispatch" {
  function_name    = "student-core-event-reminder-dispatch-${var.stage}"
  role             = var.lambda_role_arn
  runtime          = var.lambda_node_runtime
  handler          = "src/handlers/eventReminders.runScheduledReminder"
  filename         = data.archive_file.lambda_zip.output_path
  source_code_hash = data.archive_file.lambda_zip.output_base64sha256
