...
resource "aws_apigatewayv2_integration" "sms_trigger_api" {
  api_id                 = local.api_gateway_id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.sms_trigger_api.invoke_arn
  payload_format_version = "1.0"
}
...
resource "aws_lambda_permission" "sms_trigger_api" {
  statement_id  = "AllowAPIGatewayInvokeSmsTriggerApi"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.sms_trigger_api.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${local.api_execution_arn}/*/*"
}
...
resource "aws_apigatewayv2_route" "post_sms_trigger_event" {
  api_id    = local.api_gateway_id
  route_key = "POST /student/api/events/{eventId}/sms-reminder/trigger"
  target    = "integrations/${aws_apigatewayv2_integration.sms_trigger_api.id}"
}
