...
resource "aws_apigatewayv2_integration" "event_reminders_api" {
  api_id                 = local.api_gateway_id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.event_reminders_api.invoke_arn
  payload_format_version = "1.0"
}

resource "aws_apigatewayv2_integration" "event_reminder_preference" {
  api_id                 = local.api_gateway_id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.event_reminder_preference.invoke_arn
  payload_format_version = "1.0"
}
...
resource "aws_lambda_permission" "event_reminders_api" {
  statement_id  = "AllowAPIGatewayInvokeEventRemindersApi"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.event_reminders_api.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${local.api_execution_arn}/*/*"
}

resource "aws_lambda_permission" "event_reminder_preference" {
  statement_id  = "AllowAPIGatewayInvokeEventReminderPreference"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.event_reminder_preference.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${local.api_execution_arn}/*/*"
}
...
resource "aws_apigatewayv2_route" "post_student_events_proxy" {
  api_id    = local.api_gateway_id
  route_key = "POST /student/api/events"
  target    = "integrations/${aws_apigatewayv2_integration.event_reminders_api.id}"
}

resource "aws_apigatewayv2_route" "put_student_events_proxy" {
  api_id    = local.api_gateway_id
  route_key = "PUT /student/api/events/{eventId}"
  target    = "integrations/${aws_apigatewayv2_integration.event_reminders_api.id}"
}

resource "aws_apigatewayv2_route" "delete_student_events_proxy" {
  api_id    = local.api_gateway_id
  route_key = "DELETE /student/api/events/{eventId}"
  target    = "integrations/${aws_apigatewayv2_integration.event_reminders_api.id}"
}

resource "aws_apigatewayv2_route" "post_student_events_trigger_reminder" {
  api_id    = local.api_gateway_id
  route_key = "POST /student/api/events/{eventId}/trigger-reminder"
  target    = "integrations/${aws_apigatewayv2_integration.event_reminders_api.id}"
}

resource "aws_apigatewayv2_route" "get_student_event_reminder_pref" {
  api_id    = local.api_gateway_id
  route_key = "GET /student/api/events/preferences/reminders"
  target    = "integrations/${aws_apigatewayv2_integration.event_reminder_preference.id}"
}

resource "aws_apigatewayv2_route" "put_student_event_reminder_pref" {
  api_id    = local.api_gateway_id
  route_key = "PUT /student/api/events/preferences/reminders"
  target    = "integrations/${aws_apigatewayv2_integration.event_reminder_preference.id}"
}
