...
## Mentorship Embeddings Table Ownership

- The mentorship embeddings DynamoDB table is **owned and created by** the external-services module.
- Student-service **consumes** that table via the `MENTORSHIP_EMBEDDINGS_TABLE` Lambda environment variable.
- In root wiring (`infrastructure/student-core.tf`), this is passed as:
  - `${var.external_services_project_name}-mentorship-profile-embeddings`
- This keeps StudentProfiles lean while preserving isolation and centralized lifecycle management for embeddings storage.
