...
# -----------------------------------------------------------------------------
# Mentorship matching runs (admin batch orchestration audit + UI snapshot)
# PK: runId (UUID per run; plus fixed row runId = __LATEST_POINTER__)
# -----------------------------------------------------------------------------
resource "aws_dynamodb_table" "mentorship_matching_runs" {
  name         = "${var.project_name}-mentorship-matching-runs"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "runId"
