...
def diagnose_mentee_channel_counters(
    *,
    ...
    """
    Admin diagnostics for synthetic mentee opened-channel counters vs actual CHANNEL_OPENED rows.
    Useful for detecting/confirming stale counter drift.
    """
    table = _matches_table()
...
def repair_mentee_channel_counters(
    *,
    ...
    """
    Admin repair operation for synthetic mentee opened-channel counters.
    Sets synthetic counter to authoritative CHANNEL_OPENED row count.
    """
...
def accept_match(
    ...
    try:
        ...
        code = str(e)
        if code in ("match_row_unavailable", "mentee_channel_unavailable"):
            # Race-safe idempotency: if another accept already opened this exact row,
            # return success instead of surfacing a conflict to the same mentor.
            latest = table.get_item(Key=key, ConsistentRead=True).get("Item") or {}
            if str(latest.get("status") or "").upper() == "CHANNEL_OPENED":
                return {
                    "mentorUserId": mentor_user_id,
                    "menteeUserId": mentee_user_id,
                    "status": "CHANNEL_OPENED",
                    "channelId": latest.get("channelId"),
                    "acceptedAt": latest.get("acceptedAt"),
                    "suggestedIcebreaker": latest.get("suggestedIcebreaker"),
                }
        raise
    out: dict[str, Any] = {
        "mentorUserId": mentor_user_id,
