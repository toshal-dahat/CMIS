...
def reconstruct_match_table_invariants() -> dict[str, int]:
    """
    Normalize existing match rows to satisfy mentorship product invariants:
    - One active queue row (SUGGESTED/PENDING_MENTOR) per mentor at a time.
    - One CHANNEL_OPENED mentee owner at a time.
    - Any conflicting active rows become SKIPPED_BY_MENTOR.
    """
    table = _matches_table()
    rows: list[dict] = []
    scan_kw: dict[str, Any] = {
        "ProjectionExpression": "mentorUserId, menteeUserId, #st, updatedAt, createdAt, finalScore, similarityScore",
        "ExpressionAttributeNames": {"#st": "status"},
    }
    while True:
        r = table.scan(**scan_kw)
    ...
    now = _now_iso()
    closed_extra_queue_for_mentor = 0
    closed_conflicting_queue_for_mentee = 0
    closed_extra_opened_for_mentee = 0

    def _score(row: dict) -> float:
        try:
            return float(row.get("finalScore") or row.get("similarityScore") or 0.0)
        except Exception:
            return 0.0

    def _ts(row: dict) -> str:
        return str(row.get("updatedAt") or row.get("createdAt") or "")

    # Group rows (excluding synthetic counter row).
    by_mentor: dict[str, list[dict]] = defaultdict(list)
    by_mentee: dict[str, list[dict]] = defaultdict(list)
    for row in rows:
        mid = str(row.get("mentorUserId") or "").strip()
        eid = str(row.get("menteeUserId") or "").strip()
        if not mid or not eid or mid == MENTEE_CHANNEL_STATE_MENTOR_ID:
            continue
        by_mentor[mid].append(row)
        by_mentee[eid].append(row)

    # Enforce at most one active queue row per mentor.
    for mid, mrows in by_mentor.items():
        active = [r for r in mrows if str(r.get("status") or "").upper() in ("SUGGESTED", "PENDING_MENTOR")]
        if len(active) <= 1:
            continue
        active.sort(key=lambda r: (_score(r), _ts(r)), reverse=True)
        keep_key = (
            str(active[0].get("mentorUserId") or "").strip(),
            str(active[0].get("menteeUserId") or "").strip(),
        )
        for r in active[1:]:
            eid = str(r.get("menteeUserId") or "").strip()
            if not eid:
            ...
            if key == keep_key:
                continue
            table.update_item(
                Key={"mentorUserId": mid, "menteeUserId": eid},
                UpdateExpression="SET #s = :s, updatedAt = :u, skippedAt = :u, skipReason = :r",
                ExpressionAttributeNames={"#s": "status"},
                ExpressionAttributeValues={
                    ":s": "SKIPPED_BY_MENTOR",
                    ":u": now,
                    ":r": "reconstructed_single_queue_per_mentor",
                },
            )
            closed_extra_queue_for_mentor += 1

    # Enforce at most one CHANNEL_OPENED owner per mentee.
    for eid, erows in by_mentee.items():
        opened = [r for r in erows if str(r.get("status") or "").upper() == "CHANNEL_OPENED"]
        if len(opened) <= 1:
            continue
        opened.sort(key=lambda r: _ts(r), reverse=True)
        keep_mid = str(opened[0].get("mentorUserId") or "").strip()
        for r in opened[1:]:
            mid = str(r.get("mentorUserId") or "").strip()
            ...
            table.update_item(
                ...
                ExpressionAttributeValues={
                    ":s": "SKIPPED_BY_MENTOR",
                    ":u": now,
                    ":r": "reconstructed_excess_channel_opened",
                },
            )
            closed_extra_opened_for_mentee += 1

    # If a mentee has CHANNEL_OPENED, close all queue rows for other mentors.
    for eid in by_mentee.keys():
        current_rows = _query_all_matches_for_mentee(eid)
        opened = [r for r in current_rows if str(r.get("status") or "").upper() == "CHANNEL_OPENED"]
        if not opened:
            continue
        opened.sort(key=lambda r: _ts(r), reverse=True)
        keep_mid = str(opened[0].get("mentorUserId") or "").strip()
        for r in current_rows:
            mid = str(r.get("mentorUserId") or "").strip()
            st = str(r.get("status") or "").upper()
            if not mid or mid in (keep_mid, MENTEE_CHANNEL_STATE_MENTOR_ID):
                continue
            if st not in ("SUGGESTED", "PENDING_MENTOR"):
                continue
            table.update_item(
                ...
                ExpressionAttributeValues={
                    ":s": "SKIPPED_BY_MENTOR",
                    ":u": now,
                    ":r": "reconstructed_mentee_already_opened_elsewhere",
                },
            )
            closed_conflicting_queue_for_mentee += 1

    repair = repair_mentee_channel_counters(limit=2000, drift_only=False)
    return {
        "closedExtraQueueForMentor": closed_extra_queue_for_mentor,
        "closedConflictingQueueForMentee": closed_conflicting_queue_for_mentee,
        "closedExtraOpenedForMentee": closed_extra_opened_for_mentee,
        "counterRepairs": int(repair.get("repairedCount") or 0),
    }
...
def _close_other_active_rows_for_mentee(
    ...
    *,
    ...
    for row in _query_all_matches_for_mentee(uid):
        mid = str(row.get("mentorUserId") or "").strip()
        if not mid or mid in (accepted_mid, MENTEE_CHANNEL_STATE_MENTOR_ID):
            continue
        st = str(row.get("status") or "").upper()
        if st not in ("SUGGESTED", "PENDING_MENTOR"):
            continue
...
def _try_close_stale_suggestion_when_mentee_matched_elsewhere(mentor_user_id: str, mentee_user_id: str) -> bool:
    ...
    for r in _query_all_matches_for_mentee(mentee):
        ...
        if rmid in ("", MENTEE_CHANNEL_STATE_MENTOR_ID):
            continue
    ...
    *,
    ...
    try:
        ...
        if run_batch_matching:
            ...
            out["batch"] = None
        phase = "RECONSTRUCTING_MATCH_TABLE"
        out["phase"] = phase
        try:
            _persist_matching_run_phase(
                run_id,
            ...
            pass
        out["reconstruction"] = reconstruct_match_table_invariants()
