...
def do_graduation_handover(event: dict, body: dict) -> dict:
    ...
    try:
        ...
        # Ensure that signing in with the personal email resolves to the same Cognito user (same sub),
        # so the previously-stored StudentProfiles history is reused.
        _link_personal_email_to_cmis_user(user_id=sub, current_email=email, personal_email=personal_email)
    except Exception as e:
