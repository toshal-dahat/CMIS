...
def do_mentorship_mentee_request(event: dict) -> dict:
    ...
    target_mentor = _get_student_profile(mentor_user_id)
    if not _is_mentor_profile(target_mentor):
    ...
    try:
        row = mentorship_service.create_mentee_request(
            ...
            mentor_name=str(target_mentor.get("name") or "").strip() or None,
...
def _lambda_handler_impl(event: dict, context: object) -> dict:
    ...
    if isinstance(event, dict) and event.get("source") == "cmis.mentorship.batch":
        return do_mentorship_annual_batch()
