...
def do_mentorship_candidates(event: dict) -> dict:
    ...
    try:
        ...
        saved = _enrich_mentor_match_rows_with_mentee_profiles(saved)
...
def do_mentorship_mentee_matches(event: dict) -> dict:
    ...
    try:
        ...
        # Mentor-initiated program only: mentees never see in-progress suggestions (SUGGESTED / SKIPPED).
        ...
        for r in opened:
            ...
            enriched.append(row)
        return _response({"matches": enriched, "count": len(enriched)})
    except Exception as e:
