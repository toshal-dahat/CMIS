...
def do_mentorship_candidates(event: dict) -> dict:
    ...
    try:
        ...
        candidates = mentorship_service.build_mentor_candidates(mentor_profile, mentees)
        saved = mentorship_service.upsert_suggestions(sub, candidates, limit=20)
        return _response({"candidates": saved, "totalCandidates": len(saved)})
...
def do_mentorship_accept(event: dict, mentee_user_id: str) -> dict:
    ...
    try:
        accepted = mentorship_service.accept_match(sub, mentee_user_id)
...
def _lambda_handler_impl(event: dict, context: object) -> dict:
    ...
    if path_parts == ["mentorship", "embedding-config"] and method == "GET":
        try:
            ...
            return _response({"ok": True, "embeddings": provider.metadata()})
        except Exception as e:
