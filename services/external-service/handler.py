...
Cohort handoff (macro context): see HANDOFF_FOR_NEXT_TEAM.md and EXTERNAL_SERVICE_NOTES.md
in this directory before changing routing or auth behavior.
"""

# Overview:
# This file is the traffic controller for external-service:
# - parses incoming event shape
# - checks auth and role constraints
# - routes to focused helper modules
# - formats JSON HTTP responses
...
# Cognito / identity: USER_POOL_ID + CLIENT_ID are the external (Team Gig) user pool used for signup and Bearer auth.
# CMIS_* values point at the main CMIS pool so graduation handover can mirror alumni into shared CMIS groups.
USER_POOL_ID = os.environ.get("USER_POOL_ID")
CLIENT_ID = os.environ.get("CLIENT_ID")
EXTERNAL_USERS_TABLE = os.environ.get("EXTERNAL_USERS_TABLE")
# Rich student-facing profiles (mentorship, handover UIN checks) — separate from EXTERNAL_USERS_TABLE mirror of Cognito.
...
def do_mentorship_list_matches(event: dict) -> dict:
    ...
    try:
        ...
        # Hide stale SUGGESTED/PENDING rows that are no longer actionable
        # because the mentee already reached active match cap with another mentor.
        # Important: this is read-only filtering; do not mutate rows inside GET.
...
def do_mentorship_mentor_pause_put(event: dict, body: dict) -> dict:
    ...
    if day <= date.today().strftime("%Y-%m-%d"):
        return _response(
            {"error": "until must be a future date (after today)"},
            400,
        )
    student_profiles_table.update_item(
        Key={"userId": sub},
...
def do_mentorship_accept(event: dict, mentee_user_id: str) -> dict:
    ...
    body = _parse_body(event)
    idempotency_key = str((body.get("idempotencyKey") or "")).strip()[:128] or None
    ...
    try:
        ...
        if mentee_email and channel_id and not idempotent_replay:
            try:
            ...
            email_result = {"sent": False, "mode": "idempotent_replay"}
        elif not channel_id:
...
def do_post_mentorship_matching_runs(event: dict, body: dict) -> dict:
    ...
    include_snapshot = True if body.get("includeSnapshot") is True else False
    try:
...
def do_enqueue_mentorship_admin_run(event: dict, body: dict, context: object, *, trigger_source: str = "manual") -> dict:
    ...
    if bool(body.get("dryRun")):
        per_mentor_limit_raw = body.get("dryRunPerMentorLimit")
        try:
            per_mentor_limit = int(str(per_mentor_limit_raw).strip()) if per_mentor_limit_raw is not None else 1
        except Exception:
            per_mentor_limit = 1
        try:
            payload = mentorship_service.run_admin_matching_dry_run(per_mentor_limit=per_mentor_limit)
            return _response(payload, 200)
        except ValueError as e:
            if str(e) == "dry_run_disabled":
                return _response(
                    {"error": "Dry-run is disabled. Set MENTORSHIP_DRY_RUN_ENABLED=true to enable."},
                    403,
                )
            return _response({"error": str(e)}, 400)
        except Exception as e:
            return _response({"error": "Dry-run preview failed", "detail": str(e)}, 500)
    reset_matches = bool(body.get("resetMatches"))
    rb = body.get("runBatchMatching")
    run_batch_matching = True if rb is None else bool(rb)
    try:
...
def do_get_mentorship_matching_run_latest(event: dict) -> dict:
    ...
    if row.get("snapshotSummary") is None:
        row["snapshotSummary"] = {
            "pairCount": snap.get("pairCount", 0),
            "mentorCount": len(snap.get("mentors") or []),
        }
...
def do_get_mentorship_admin_live_snapshot(event: dict) -> dict:
    """GET /mentorship/admin/matches-live"""
    if not _is_requester_admin(event):
        return _response({"error": "Admin access required"}, 403)
    try:
        snap = mentorship_service.build_admin_run_snapshot()
        return _response({"snapshot": snap}, 200)
    except Exception as e:
        return _response({"error": "Failed to build live admin snapshot", "detail": str(e)}, 500)
...
def do_post_mentorship_admin_pair_suggest_by_email(event: dict, body: dict) -> dict:
    """
    POST /mentorship/admin/pair-suggest-by-email
    Admin-only. Body: { "mentorEmail": string, "menteeEmail": string }
    Resolves profile userIds from email aliases, then performs pair-suggest reset flow.
    """
    if not _is_requester_admin(event):
        return _response({"error": "Admin access required"}, 403)
    mentor_email = str((body.get("mentorEmail") or "")).strip().lower()
    mentee_email = str((body.get("menteeEmail") or "")).strip().lower()
    if not mentor_email or not mentee_email:
        return _response({"error": "mentorEmail and menteeEmail are required"}, 400)
    mentor_profile = _get_student_profile_by_any_email(mentor_email)
    mentee_profile = _get_student_profile_by_any_email(mentee_email)
    mentor_uid = str((mentor_profile or {}).get("userId") or "").strip()
    mentee_uid = str((mentee_profile or {}).get("userId") or "").strip()
    if not mentor_uid:
        return _response({"error": "mentor_profile_not_found_by_email", "mentorEmail": mentor_email}, 404)
    if not mentee_uid:
        return _response({"error": "mentee_profile_not_found_by_email", "menteeEmail": mentee_email}, 404)
    try:
        result = mentorship_service.admin_reset_mentee_and_suggest_to_mentor(mentor_uid, mentee_uid)
        result["resolved"] = {
            "mentorUserId": mentor_uid,
            "mentorEmail": mentor_email,
            "menteeUserId": mentee_uid,
        ...
        return _response({"error": "pair_suggest_by_email_failed", "detail": str(e), "ok": False}, 500)


def do_post_mentorship_admin_recycle_expired(event: dict, body: dict) -> dict:
    """
    POST /mentorship/admin/recycle-expired
    Admin-only. Body: { "limit"?: number, "autoReassign"?: boolean }
    Closes TTL-expired SUGGESTED/PENDING_MENTOR rows and optionally tries auto-reassign.
    """
    if not _is_requester_admin(event):
        return _response({"error": "Admin access required"}, 403)
    try:
        ...
        limit = 200
    auto_reassign = True if body.get("autoReassign") is None else bool(body.get("autoReassign"))
    try:
        result = mentorship_service.recycle_expired_suggestions(limit=limit, auto_reassign=auto_reassign)
        return _response(result, 200)
    except Exception as e:
        return _response({"error": "recycle_expired_failed", "detail": str(e)}, 500)
...
#
# Current CMIS "direct handover" path (signed-in user):
# - validates caller + profile existence
# - optionally verifies provided UIN matches StudentProfiles.uin
# - updates StudentProfiles role/email aliases for alumni handover
# - syncs CMIS Cognito alumni/friends groups
# NOTE: this route intentionally does not replace Cognito `email` with personal email.
# ---------------------------------------------------------------------------
...
# Signed-in pre-check for UI confirmation before POST /graduation-handover.
# Returns limited student profile fields only; this endpoint does not link/claim.
# ---------------------------------------------------------------------------
...
def _lambda_handler_impl(event: dict, context: object) -> dict:
    # Non-HTTP invokes use `source` (EventBridge, async self-invoke, cross-service) instead of API Gateway requestContext.
    # Run these branches before the generic `aws.events` graduation scan so mentorship jobs are not mistaken for scan.
    ...
    # HTTP API v2 (and REST) paths normalized to segments, e.g. /mentorship/candidates -> ["mentorship", "candidates"].
    ...
    if path_parts == ["graduation-handover", "request-link"] and method == "POST":
        # Email-first flow: generate and deliver magic link for eligible graduates.
        email = (body.get("email") or "").strip().lower()
    ...
    if path_parts == ["graduation-handover", "claim"] and method == "GET":
        # Claim step 1: validate token and return context for claim UI.
        try:
            query = event.get("queryStringParameters") or {}
    ...
    if path_parts == ["graduation-handover", "claim"] and method == "POST":
        # Claim step 2: consume token, create/reuse account, and link UIN.
    ...
    if path_parts == ["mentorship", "admin", "matches-live"] and method == "GET":
        return do_get_mentorship_admin_live_snapshot(event)
    ...
    if path_parts == ["mentorship", "admin", "pair-suggest-by-email"] and method == "POST":
        return do_post_mentorship_admin_pair_suggest_by_email(event, body)
    if path_parts == ["mentorship", "admin", "recycle-expired"] and method == "POST":
        return do_post_mentorship_admin_recycle_expired(event, body)
