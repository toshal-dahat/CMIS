...
def do_mentorship_candidates(event: dict) -> dict:
    ...
    try:
        ...
        saved = mentorship_service.upsert_suggestions(sub, candidates, mentor_display_name=mname)
...
def do_mentorship_list_matches(event: dict) -> dict:
    ...
    try:
        ...
        return _response({"matches": rows, "count": len(rows)})
    except Exception as e:
...
def do_mentorship_mentee_request(event: dict) -> dict:
    ...
    body = _parse_body(event)
    mentor_user_id = str(body.get("mentorUserId") or "").strip()
    if not mentor_user_id:
        return _response({"error": "mentorUserId is required"}, 400)
    if mentor_user_id == sub:
        return _response({"error": "Cannot request yourself"}, 400)

    mentor_profile = _get_student_profile(mentor_user_id)
    if not _is_mentor_profile(mentor_profile):
        return _response({"error": "Target user is not an available mentor"}, 404)

    try:
        row = mentorship_service.create_mentee_request(
            sub,
            mentor_user_id,
            mentee_name=str(mentee_profile.get("name") or "").strip() or None,
            mentor_name=str(mentor_profile.get("name") or "").strip() or None,
            mentee_email=(email or "").strip() or None,
        )
        return _response({"request": row}, 201)
    except ValueError as e:
        if str(e) == "already_matched":
            return _response({"error": "You are already matched with this mentor"}, 409)
        return _response({"error": str(e)}, 400)
    except Exception as e:
        return _response({"error": "Failed to create mentorship request", "detail": str(e)}, 500)
...
def _lambda_handler_impl(event: dict, context: object) -> dict:
    ...
    if path_parts == ["mentorship", "mentee", "request"] and method == "POST":
        return do_mentorship_mentee_request(event)
