"""Board-tier boost resolver for mentor ranking.

Overview:
- We look at a mentor's email/company details.
- We map that to a tier (`gold`, `silver`, `bronze`, or admin tier slug).
- We return a multiplier that can boost that mentor's score.
- We also return a human-readable reason so UI/admin tools can explain it.
...
def _company_cache_ttl_sec() -> float:
    """How long company/tier API responses are cached in-memory."""
    try:
...
def _tiers_list_url() -> str:
    """Find the tiers endpoint, or derive it from the companies URL."""
...
def _domain_from_email(email: str) -> str:
    """Extract domain from email; return empty string if invalid."""
...
def _normalize_tier_value(raw: Any) -> str:
    """Normalize tier aliases (e.g. `board_gold`) to canonical slugs."""
    if raw is None:
        return "none"
...
def _display_slug(raw: str) -> str:
    """Normalize a tier id into a comparison-friendly slug."""
...
def _cached_company_fetch() -> tuple[list[dict], float]:
    """Return `(companies, fetched_at_monotonic)` using cache when possible."""
...
def _cached_tier_items() -> tuple[dict[str, Any], ...]:
    """Return tier rows from admin API with short TTL cache."""
...
def _tier_slug_multipliers() -> dict[str, float]:
    """Build multiplier values from tier ranking.

    Highest-ranked tier gets the configured "top" multiplier, lowest-ranked tier
    gets the "bottom" multiplier, and middle tiers are evenly spaced between.
    """
...
def build_board_lookup() -> tuple[dict[str, BoardEntry], dict[str, BoardEntry]]:
    """Build lookup maps used during mentor-tier resolution.

    Returns:
    - domain map: `domain -> (tier_slug, rank)`
    - company-name map: `name -> (tier_slug, rank)`
    """
...
def _fallback_match_mult() -> float:
    """Fallback multiplier when no usable tiers metadata is available."""
    try:
...
def _mult_for_slug(slug: str, mult_map: dict[str, float]) -> tuple[float, str]:
    """Resolve multiplier + explanation for one tier slug."""
...
def resolve_mentor_board_tier(mentor_profile: dict | None) -> tuple[str, float, str]:
    ...
    Matching order:
    1) mentor email domain
    2) exact mentor company-name match
    3) partial company-name/domain-like match
    4) default no-boost
    """
...
def clear_board_cache() -> None:
    """Clear cached company/tier responses (mainly useful in tests)."""
...
def get_boost_multiplier(mentor_profile: dict | None = None) -> float:
    """Backward-compatible helper: return only the numeric multiplier."""
