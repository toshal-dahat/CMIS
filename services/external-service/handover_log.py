"""Append-only audit trail for graduation handover (INITIATED / SUCCESS / FAILED).

Each ``put_item`` writes a new row keyed by ``handover_id`` + ``timestamp`` so operators can
correlate a flow without mutating prior events. DynamoDB TTL attribute ``ttl_expiry`` prunes
old rows after ~90 days. If ``HANDOVER_LOG_TABLE`` is unset, all functions no-op safely.
"""
...
TABLE_NAME = (os.environ.get("HANDOVER_LOG_TABLE") or "").strip()
...
def log_initiated(user_id: str, uin: str, personal_email: str) -> str:
    ...
    try:
        ...
        # Never block the handover path on optional observability.
        pass
    return handover_id
...
def log_success(handover_id: str, user_id: str, uin: str) -> None:
    ...
    try:
        ...
        pass  # optional sink — ignore Dynamo/permission errors
