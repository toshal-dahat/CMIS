...
- **Graduation handover**: graduation prompt + direct UIN linking to transition students to alumni access
...
## Graduation handover workflow (actual current app flow)
...
| `HANDOVER_TOKENS_TABLE` | DynamoDB table name retained for legacy compatibility | `cmis-handover-tokens` |
...
| `FRONTEND_BASE_URL` | Base URL used by frontend integrations | `https://app.example.com` |
...
| `SES_VERIFIED_SENDER` | Verified SES email for notification messages | Emails skipped if unset |
