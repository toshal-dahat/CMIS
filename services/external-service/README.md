...
| `cmis.mentorship.batch` | Annual greedy batch: one `SUGGESTED` row per mentee to best available mentor (`run_annual_batch_matching`) |
...
| `MENTORSHIP_EMBEDDINGS_PROVIDER` | **`bedrock-titan`** (default), `bedrock-cohere`, or `openai` (legacy) |
...
| `OPENAI_API_KEY` | Required only when `MENTORSHIP_EMBEDDINGS_PROVIDER=openai` |
| `OPENAI_EMBEDDING_MODEL` | OpenAI embeddings model when using OpenAI |
...
The root **Deploy CMIS Application** workflow may pass **`OPENAI_API_KEY`** when using OpenAI embeddings. For the default **Bedrock** path, ensure the Lambda role allows `bedrock:InvokeModel` / `bedrock:Converse` on the configured models (see `infrastructure/external-services/terraform/iam.tf`).
