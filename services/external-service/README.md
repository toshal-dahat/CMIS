...
| POST | `/mentorship/mentee/request` | Mentee-only: body `{ "mentorUserId" }` → `PENDING_MENTOR` |
...
## Graduation handover workflows

The service currently supports two handover paths that converge on the same alumni state (`role = FORMER_STUDENT` on StudentProfiles and CMIS alumni/friends group sync).

### 1) Signed-in direct handover (in-app)

Use when a logged-in user is still on their student identity and wants to add a personal email.

1. `GET /graduation-handover/lookup?uin=` (optional verification step for UI confirmation).
2. `POST /graduation-handover` with `personalEmail` (and optional `uin` match check).
3. Backend updates StudentProfiles role/email aliases and ensures CMIS group membership for alumni/friends.

Implementation path:
- [`handler.py`](handler.py): `do_handover_lookup`, `do_graduation_handover`
- [`handler.py`](handler.py): `_update_student_profile_alumni`, `_ensure_cmis_alumni_membership`

### 2) Magic-link claim handover (email-first)

Use when a graduate starts from personal email claim instead of an active signed-in student session.

1. Link issued by either:
   - scheduled graduation scan (EventBridge -> `run_scan`), or
   - self-service `POST /graduation-handover/request-link`.
2. `GET /graduation-handover/claim?token=` validates token and returns claim context.
3. `POST /graduation-handover/claim` validates token again, creates/reuses account, links UIN, marks token `claimed=true`.

Implementation path:
- [`graduation_scan.py`](graduation_scan.py): `run_scan`, `request_magic_link_for_email`
- [`graduation_claim.py`](graduation_claim.py): `get_token_info`, `claim_with_password`
- [`handover.py`](handover.py): `link_uin_to_user` (role + linked UIN persistence)

### Notes / behavior guarantees

- Magic-link tokens are hashed in DynamoDB and expire (7-day TTL configured in code).
- Claim tokens are single-use (`claimed` flag prevents replay).
...
| `cmis.mentorship.batch` | Annual batch: partner-tier mentors (gold/silver/bronze + ranked tiers) are prioritized for first coverage, then one `SUGGESTED` row per mentee is assigned with partner-first + low-load tie-breaking (`run_annual_batch_matching`); response now includes `unmatchedMentees`, `unmatchedMentors`, and per-reason counts |
| `cmis.mentorship.late-registration` | Body must include `menteeUserId`; runs `run_single_mentee_matching` (used from student-service on profile create / switch to mentee) |

Terraform can define **Sept 1, 06:00 UTC** EventBridge → Lambda with `source: cmis.mentorship.batch` (`infrastructure/external-services/terraform/eventbridge.tf`). This is **optional** and **disabled by default** (`enable_mentorship_annual_eventbridge = false`) because many Terraform IAM users lack `events:PutRule`. Set the variable to `true` after granting EventBridge permissions, or invoke the Lambda manually with the same JSON payload.
...
| `SES_VERIFIED_SENDER` | (Optional) Verified SES email for magic-link, accept, and soft decline notifications |
