...
| GET | `/mentorship/matches` | Mentor-only: list persisted mentorship rows (pending first); same hydration as candidates |
| GET | `/mentorship/mentee/matches` | Mentee-only: **confirmed** matches (`CHANNEL_OPENED` only); response includes mentor profile contact fields loaded from StudentProfiles |
