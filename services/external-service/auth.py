...
# Overview:
# Think of this file as a thin wrapper around Cognito API calls.
# It keeps "how to talk to Cognito" in one place so handler/business files
# can focus on app logic instead of raw AWS request shapes.
...
def admin_set_custom_attributes(username: str, role: str, class_year: Optional[str] = None, linked_uin: Optional[str] = None) -> None:
    ...
    try:
        ...
        # Older pools may omit custom:* attributes; external-users Dynamo remains source of truth for role/UIN/year.
        pass
