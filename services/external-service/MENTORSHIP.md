...
- **Embeddings (default):** Amazon **Bedrock** Titan Text Embeddings v2 (`amazon.titan-embed-text-v2:0`), env `MENTORSHIP_EMBEDDINGS_PROVIDER=bedrock-titan`. Alternative: `bedrock-cohere` (Cohere English v3 on Bedrock).
...
- **Default quality/price:** Bedrock Titan Text Embeddings v2
- **Alternative:** Bedrock Cohere English v3
- **Optional self-hosted option:** `bge-large-en-v1.5` (or similar strong retrieval embedding model)

For initial rollout, start with one Bedrock embedding model and offline evaluation on historical sample data before production auto-matching.
...
- Added `services/external-service/mentorship_embeddings.py`:
  ...
  - Provider selection through env vars:
    - `MENTORSHIP_EMBEDDINGS_PROVIDER` (default `bedrock-titan`)
