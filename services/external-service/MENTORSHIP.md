...
- **Mentee active match cap**: a student may have at most **N** concurrent `CHANNEL_OPENED` matches with **distinct** mentors (default **N=3**, env `MENTORSHIP_MENTEE_MAX_ACTIVE_MATCHES`). Mentees at the cap are omitted from `GET /mentorship/candidates`; `POST .../accept` returns **409** with `mentee_at_match_cap` if accepting would exceed the cap. Accept is idempotent for an already-open channel (no duplicate notification email).
