...
const STUDENT_GROUPS = ['students', 'alumni', 'friends'];
// Groups that bypass the capacity limit
...
class RsvpService {
    ...
    async rsvpToEvent(eventId, userId, userEmail, userGroups) {
        ...
        const isPartner  = groups.some(g => g.toLowerCase() === 'investors');
        ...
        // Students/friends bypass the time gate entirely.
        // Partners are gated by their company tier's earlyAccessHours.
        // Admins are exempt from all gating.
        if (!isStudent && !isAdmin && isPartner) {
    ...
    async _fetchPartnerEarlyAccessHours(userEmail) {
        try {
            ...
            if (domainRes.status === 404) return 0; // Not a partner domain — no gate
            if (!domainRes.ok) {
