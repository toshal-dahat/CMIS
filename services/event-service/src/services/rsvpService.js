...
class RsvpService {
    ...
    async rsvpToEvent(eventId, userId, userEmail) {
        ...
        // Add a 48-hour base General Admission window so standard students can actually RSVP before 
        // the event starts. Then, add the VIP's bonus early access hours on top of that base window.
        const GENERAL_ADMISSION_HOURS = 48;
        const totalUnlockHours = GENERAL_ADMISSION_HOURS + earlyAccessHours;

        const eventStartTime = new Date(event.date).getTime();
        const unlockTime = eventStartTime - (totalUnlockHours * 60 * 60 * 1000);
        if (Date.now() < unlockTime) {
            const e = new Error(`You cannot RSVP for this event until ${new Date(unlockTime).toLocaleString()}.`);
            e.statusCode = 403;
            throw e;
        }
        // -----------------------------------------------------
