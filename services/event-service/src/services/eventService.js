...
class EventService {
    ...
    /**
     * Update an existing event using DynamoDB Conditional Writes (Optimistic Locking).
     *
     * This prevents race conditions during high-traffic updates (e.g., RSVPs).
     * The client must pass the current `version` of the event. The update will only
     * succeed if the database `version` exactly matches the expected `version`.
     * Upon success, the `version` is incremented by 1.
     *
     * @param {string} eventId - The UUID of the event.
     * @param {Object} updateData - Partial event data (must include `version` for optimistic locking).
     * @throws {Error} Throws a 409 Conflict if the ConditionExpression fails.
     */
