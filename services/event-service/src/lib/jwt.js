...
async function requireAuth(req, res, next) {
    ...
    // When COGNITO_USER_POOL_ID is absent (local dev, CI without Cognito), skip real JWT
    // verification. First try to Base64-decode the token as a real JWT payload — demo scripts
    // and some test harnesses generate fake tokens with proper claim shapes (sub, email, groups).
    // If that fails, fall back to granting admin rights to the literal string 'admin-token'
    // so tests can exercise admin-gated routes without spinning up a real Cognito user pool.
    if (!process.env.COGNITO_USER_POOL_ID) {
        try {
            ...
            // Ignore parsing errors and fall through to the simple-token fallback below.
        }

        // 'admin-token' grants the 'admins' group so test-auth-endpoints.js can reach admin routes.
        // Any other token is treated as a regular authenticated user with no groups.
...
function requireAdmin(req, res, next) {
    ...
    // Cognito group names are case-sensitive and were created inconsistently across environments.
    // This list covers all known casings. 'SuperAdmin' / 'superadmin' / 'SUPERADMIN' map to the
    // higher-privilege tier defined in admin-service but should carry the same rights here.
