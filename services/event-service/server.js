/**
 * Local Development Server - Event Service
 * 
 * This server allows for local development and testing of the Event Service
 * without requiring connection to AWS resources. 
 * 
 * Core Mechanics:
 * 1. In-Memory Store: Uses JS Maps to simulate DynamoDB tables.
 * 2. Monkey-Patching: Intercepts the `@aws-sdk/lib-dynamodb` and `@aws-sdk/client-dynamodb`
 *    modules to redirect database calls to the in-memory mock.
 * 3. Feature Simulation: Implements basic logic for:
 *    - Optimistic Locking (via `version` field).
 *    - Conditional Checks (`attribute_not_exists`, capacity guards).
 *    - Transactions (`TransactWriteCommand`).
 *    - GSI Simulation (via `QueryCommand` filtering).
 * 
...
/**
 * Mock implementation of the DynamoDB Document Client's send method.
 * Handles parsing command inputs and updating the in-memory store.
 */
const mockDocClient = {
   send(command) {
...
// Velvet Rope Mock Configuration for Demo Purposes
app.get('/mock/domain/:domain', (req, res) => {
   const domain = req.params.domain.toLowerCase();
   if (domain === 'goldpartner.com') return res.json({ tierId: 'gold' });
   if (domain === 'silverpartner.com') return res.json({ tierId: 'silver' });
   // If it's standard.com or any other domain, it simulates standard access
...
app.get('/mock/config', (req, res) => {
   res.json({
      tiers: [
         { tierId: 'gold', earlyAccessHours: 48, rank: 1 },
         { tierId: 'silver', earlyAccessHours: 24, rank: 2 }
      ]
   });
});
