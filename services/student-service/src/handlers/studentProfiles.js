...
async function crud(event) {
  try {
    ...
    if (method === "PUT" && pathIncludesMe) {
      ...
      const patch = { ...body };
      patch.mentorshipInterested = merged.mentorshipInterested;
      patch.mentorship = merged.mentorship;
      patch.mentorCapacity = merged.mentorCapacity;

      const profile = await studentProfilesService.update(userId, patch);
