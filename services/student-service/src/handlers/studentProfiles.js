...
async function crud(event) {
  try {
    ...
    if (method === "PUT" && pathIncludesMe) {
      const body = JSON.parse(event.body ?? "{}");
      // Resolve the row to update the same way as GET /profiles/me (email aliases first, then Cognito sub).
      // Otherwise updates can 404 or write a different row than GET returns when sub and profile userId diverge.
      const loginEmail = (claims?.email ?? "").toLowerCase();
      let existing = null;
      if (loginEmail) {
        existing = await studentProfilesService.getByAnyEmail(loginEmail);
      }
      if (!existing) {
        existing = await studentProfilesService.getByUserId(userId);
      }
      if (!existing) {
        return jsonResponse(404, { error: "NOT_FOUND", message: "Profile not found" });
      }
      ...
      const profile = await studentProfilesService.update(targetUserId, body);
