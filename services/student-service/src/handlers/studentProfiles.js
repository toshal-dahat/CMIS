...
const {
  ...
  getEmbeddingRecord,
} = require("../services/mentorshipEmbeddingService");
function triggerLateRegistrationMatching(_profile) {}
function triggerMentorshipEmbeddingPrecompute(_profile) {}

async function runProfileEmbeddingWithTimeout(profile, triggerSource, timeoutMs = 1200) {
  try {
    const userId = String(profile?.userId || profile?.userSub || "").trim();
    if (!userId) return;
    const embeddingTask = embedAndPersistProfile(userId, profile, triggerSource).catch((e) => {
      console.warn("[mentorship-embed] non-fatal profile embedding error:", e?.message || e);
    });
    await Promise.race([
      embeddingTask,
...
async function crud(event) {
  try {
    ...
    if (method === "GET" && path.includes("/profiles/me/mentorship-embedding-preview")) {
      const profile = await resolveCurrentProfile(userId, claims);
      if (!profile) {
        return jsonResponse(404, { error: "NOT_FOUND", message: "Profile not found" });
      }
      const preview = getEmbeddingTemplatePreview(profile);
      const embedding = await getEmbeddingRecord(String(profile.userId || userId));
      return jsonResponse(200, {
        modelId: preview.modelId,
        mentorText: preview.mentorText,
        menteeText: preview.menteeText,
        mentorshipEmbeddingMeta: embedding?.mentorshipEmbeddingMeta ?? null,
        mentorEmbeddingTextHash: embedding?.mentorEmbeddingTextHash ?? null,
        menteeEmbeddingTextHash: embedding?.menteeEmbeddingTextHash ?? null,
        mentorEmbeddingDimensions: embedding?.mentorEmbeddingDimensions ?? null,
        menteeEmbeddingDimensions: embedding?.menteeEmbeddingDimensions ?? null,
      });
    }
    ...
    if (method === "POST" && path.includes("/profiles") && !pathIncludesMe) {
      ...
      await runProfileEmbeddingWithTimeout(profile, "profile_create");
    ...
    if (method === "PUT" && pathIncludesMe) {
      ...
      triggerMentorshipEmbeddingPrecompute(profile);
      await runProfileEmbeddingWithTimeout(profile, "profile_update");
