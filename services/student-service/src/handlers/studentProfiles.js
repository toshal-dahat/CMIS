...
function parseSmsOptInAndPhone(target) {
  const hasSmsOptIn = target.smsOptIn !== undefined;
  const hasPhone = target.phoneNumber !== undefined;
  if (!hasSmsOptIn && !hasPhone) return { ok: true };

  if (hasSmsOptIn && typeof target.smsOptIn !== "boolean") {
    return {
      ok: false,
      statusCode: 400,
      message: "smsOptIn must be a boolean.",
    };
  }

  if (hasPhone) {
    const raw = String(target.phoneNumber ?? "").trim();
    target.phoneNumber = raw || null;
    if (target.smsOptIn === true && !target.phoneNumber) {
      return {
        ok: false,
        statusCode: 400,
        message: "phoneNumber is required when smsOptIn is true.",
      };
    }
  }

  if (target.smsOptIn === false) {
...
async function crud(event) {
  try {
    ...
    if (method === "POST" && path.includes("/profiles") && !pathIncludesMe) {
      ...
      const smsPost = parseSmsOptInAndPhone(body);
      if (!smsPost.ok) {
    ...
    if (method === "PUT" && pathIncludesMe) {
      ...
      const smsPut = parseSmsOptInAndPhone(merged);
      if (!smsPut.ok) {
      ...
      if (body.smsOptIn !== undefined || body.phoneNumber !== undefined) {
        patch.smsOptIn = merged.smsOptIn;
        patch.phoneNumber = merged.phoneNumber;
      }
