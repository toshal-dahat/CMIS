...
} = require("../services/mentorshipLateRegistration");

function triggerLateRegistrationMatching(profile) {
  try {
    // Payload source: "late-registration"; InvocationType: "Event" (async fire-and-forget).
    if (!profile) return;
    if (profile.mentorshipInterested === true && String(profile.mentorship ?? "").toLowerCase() === "mentee") {
      void fireAndForgetMentorshipMatching(profile.userId || profile.userSub || "");
    }
  } catch (e) {
    console.warn("[mentorship] triggerLateRegistrationMatching failed:", e?.message || e);
  }
}

function triggerMentorshipEmbeddingPrecompute(profile) {
  try {
    if (!profile) return;
    const uid = String(profile.userId || profile.userSub || "").trim();
    if (!uid) return;
    const interested = profile.mentorshipInterested === true;
    const role = String(profile.mentorship ?? "").toLowerCase();
    if (interested && (role === "mentor" || role === "mentee")) {
      void fireAndForgetMentorshipEmbeddingPrecompute(uid);
    }
  } catch (e) {
    console.warn("[mentorship] triggerMentorshipEmbeddingPrecompute failed:", e?.message || e);
  }
}
...
function normalizePhoneE164(rawValue) {
  // Normalize/validate phone numbers to E.164 so SNS delivery is reliable.
  const value = String(rawValue ?? "").trim();
  if (!value) return null;
  if (!/^\+[1-9]\d{6,14}$/.test(value)) return { error: "Phone number must be in E.164 format (example: +19795551234)." };
  return value;
}

function applySmsPreferenceDefaultsAndValidation(target) {
  // Always persist a concrete boolean for opt-in state.
  const optIn = target.smsOptIn === true;
  target.smsOptIn = optIn;

  const normalizedPhone = normalizePhoneE164(target.phoneE164);
  if (normalizedPhone && typeof normalizedPhone === "object" && normalizedPhone.error) {
    return { ok: false, statusCode: 400, message: normalizedPhone.error };
  }
  target.phoneE164 = typeof normalizedPhone === "string" ? normalizedPhone : null;

  if (optIn && !target.phoneE164) {
    return { ok: false, statusCode: 400, message: "Phone number is required when SMS notifications are enabled." };
  }
...
async function crud(event) {
  try {
    ...
    if (method === "POST" && path.includes("/profiles") && !pathIncludesMe) {
      ...
      // SMS preference validation lives with profile create/update payload handling.
      const smsPost = applySmsPreferenceDefaultsAndValidation(body);
      if (!smsPost.ok) {
        return jsonResponse(smsPost.statusCode, {
          error: "BAD_REQUEST",
          message: smsPost.message,
        });
      }

      const profile = await studentProfilesService.create(userId, body);

      triggerLateRegistrationMatching(profile);
      triggerMentorshipEmbeddingPrecompute(profile);
    ...
    if (method === "PUT" && pathIncludesMe) {
      ...
      // Re-validate SMS fields against merged profile state for partial updates.
      const smsPut = applySmsPreferenceDefaultsAndValidation(merged);
      if (!smsPut.ok) {
        return jsonResponse(smsPut.statusCode, {
          error: "BAD_REQUEST",
          message: smsPut.message,
        });
      }
      const patch = { ...body };
      ...
      patch.smsOptIn = merged.smsOptIn;
      patch.phoneE164 = merged.phoneE164;
