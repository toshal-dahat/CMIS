...
function normalizeExtraction(parsed) {
  ...
  const education = Array.isArray(parsed?.education)
    ? parsed.education
        ...
        .filter((e) => e.institution || e.degree || e.field || e.details || e.dates)
    : [];
...
function buildPrimaryExtractionPrompt(ocrText) {
  return [
    ...
    "- gpa: number | null — only if an overall GPA appears explicitly; otherwise null.",
    '- location: string | null — city/region/country or mailing location if clearly stated; else null.',
    "- education: array of objects { institution, degree, field, dates, details } — strings; use \"\" for unknown subfields.",
...
function buildRetryExtractionPrompt(ocrText) {
  return [
    ...
    "Include keys: skills, gpa, location, education, experience, projects, achievements (same rules as the primary prompt).",
