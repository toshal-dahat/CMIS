...
const { embedAndPersistProfile } = require("../services/mentorshipEmbeddingService");
...
async function processRecord(bucket, s3Key) {
  ...
  try {
    ...
    try {
      const merged = await studentProfilesService.mergeExtractionIntoProfile(userSub, {
        gpa: extracted.gpa,
        education: extracted.education,
        skillKeys,
      });
      if (merged?.userId) {
        await embedAndPersistProfile(String(merged.userId), merged, "resume_extraction");
      }
    } catch (mergeErr) {
