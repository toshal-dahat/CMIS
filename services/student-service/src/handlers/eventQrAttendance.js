...
async function extractEventIdFromScan(payload) {
  ...
  // Camera apps may scan the QR deep-link URL instead of a bare token.
  if (/^https?:\/\//i.test(raw)) {
    try {
      const u = new URL(raw);
      const urlToken = String(u.searchParams.get("checkinToken") || "").trim();
      const urlEventId = String(u.searchParams.get("eventId") || "").trim();
      if (urlToken) raw = urlToken;
      else if (urlEventId) return urlEventId;
    } catch (_) {
      // ignore malformed URL, keep fallback behavior
    }
  }

  // Prefer signed event-checkin token format from generated event QR.
  if (raw.split(".").length === 3) {
    try {
      const decoded = jwt.verify(raw, QR_SIGNING_SECRET, {
        algorithms: ["HS256"],
      ...
      if ((tokenType === "event-checkin" || !tokenType) && eventId) {
        return eventId;
      }
    } catch (_) {
      // Not a valid signed event token.
    }
  }

  // Fallback: raw event ID scan.
  return raw;
}
...
async function generateEventQr(event) {
  try {
    ...
    // Stable per-event token: deterministic payload + no iat/exp/jti.
    const tokenPayload = {
      typ: "event-checkin",
      eventId,
    };
  ...
  try {
    ...
    const rsvpRes = await docClient.send(
      new GetCommand({
        ...
        Key: { eventId, userId: scannedUserId },
      })
    );
    ...
    if (!rsvpRes.Item) {
      return jsonResponse(404, {
        ...
        message: "User is not RSVP'd for this event.",
      });
    }

    const currentStatus = String(rsvpRes.Item.status || "").toUpperCase();
    if (currentStatus !== "CONFIRMED") {
      return jsonResponse(409, {
        status: "NOT_ELIGIBLE",
        eventId,
        userId: scannedUserId,
        message: `User RSVP status is ${currentStatus || "UNKNOWN"}, not eligible for check-in.`,
      });
    }

    if (rsvpRes.Item.checkedInAt || rsvpRes.Item.checkedIn === true) {
      return jsonResponse(200, {
        status: "ALREADY_CHECKED_IN",
        eventId,
        userId: scannedUserId,
        checkedInAt: rsvpRes.Item.checkedInAt || null,
    ...
    try {
      await docClient.send(
        new UpdateCommand({
          ...
          Key: { eventId, userId: scannedUserId },
          UpdateExpression:
            "SET checkedIn = :trueVal, checkedInAt = :checkedInAt, checkedInBy = :checkedInBy, checkInSource = :source, attendanceStatus = :attendanceStatus",
          ConditionExpression:
            "attribute_exists(eventId) AND attribute_exists(userId) AND #status = :confirmed AND attribute_not_exists(checkedInAt)",
      ...
      if (err.name === "ConditionalCheckFailedException") {
        ...
        return jsonResponse(409, {
          ...
          eventId,
          userId: scannedUserId,
          message: "User is no longer eligible for check-in.",
        });
      }
      throw err;
    }
  ...
  try {
    ...
    if (!rsvpRes.Item) {
      return jsonResponse(404, {
        ...
        eventId,
        userId,
        message: "You are not RSVP'd for this event.",
      });
    }
    ...
    if (currentStatus !== "CONFIRMED") {
      return jsonResponse(409, {
        ...
        eventId,
        userId,
        message: `Your RSVP status is ${currentStatus || "UNKNOWN"}, not eligible for check-in.`,
      });
    }
