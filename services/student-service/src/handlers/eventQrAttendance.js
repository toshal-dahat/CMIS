...
// ---------- Environment + feature constants ----------
// These defaults keep local/dev behavior predictable while allowing Terraform/env
// to override production table names and signing settings.
...
// ---------- Response helpers ----------
// Centralized response builders keep CORS headers and payload shapes consistent
// across all endpoints in this handler file.
...
// ---------- Authorization + request parsing helpers ----------
// Small helpers in this section normalize claims/headers/path data and make
// endpoint handlers easier to read.
...
// ---------- Event-day gating helpers ----------
// Check-in is intentionally constrained to the event calendar day in a configured
// timezone, so "today" is interpreted consistently for all scanners.
...
// ---------- QR payload classification + token decoding ----------
// Scans can come in as raw text, URLs, or JWT payloads. This block normalizes
// those forms into canonical event/user identifiers before business validation.
/**
...
// Admin scanner accepts multiple scan formats for attendee identity:
// explicit userId, JSON payload, signed custom token, Cognito token, or raw text fallback.
...
// ---------- Data access helpers ----------
// Keep Dynamo reads in one place so endpoint handlers are focused on flow logic.
...
// ---------- Check-in write path ----------
// This is the single source of truth for persisting attendance. Both admin and
// self check-in call this function to guarantee identical write semantics.
/**
...
// ---------- Endpoint: Generate event QR (admin) ----------
// Produces a stable per-event signed payload and QR image used by both scanner modes.
...
// ---------- Endpoint: Admin scanner check-in ----------
// Handles admin-operated scans against a specific event path parameter.
...
// ---------- Endpoint: Self check-in ----------
// Allows authenticated attendees to scan an event QR and check themselves in,
// including event-id and QR-date mismatch safeguards.
...
// ---------- Endpoint: Hosted scanner page ----------
// Returns a lightweight mobile-friendly scanner UI that posts to this handler's
// check-in endpoints. Kept here to avoid separate frontend deployment coupling.
