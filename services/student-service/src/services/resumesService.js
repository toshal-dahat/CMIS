...
function getTableName() {
  // Validate env lazily so tests/startup can load module without immediate throw.
  if (!TABLE_NAME) {
...
async function create(userSub, { resumeId, s3Key, fileName, contentType }) {
  ...
  // Initial write created before client upload completes; status advances later.
  const item = {
    userSub,
    resumeId,
    s3Key,
...
async function markUploaded(userSub, resumeId, { fileSize, etag }) {
  const now = new Date().toISOString();
  // Upload completion checkpoint used by downstream extraction trigger/polling.
  const result = await docClient.send(
    new UpdateCommand({
      TableName: getTableName(),
      Key: { userSub, resumeId },
 ...
 * Resume Parser bounty storage contract:
 * - status transitions to EXTRACTED
 * - extractedData is normalized to a stable shape consumed by polling UIs
 */
async function markExtracted(userSub, resumeId, extracted) {
  const now = new Date().toISOString();
  // Store a stable extractedData shape so clients can render without field guards.
  const result = await docClient.send(
    new UpdateCommand({
      TableName: getTableName(),
      Key: { userSub, resumeId },
...
async function markExtractionFailed(userSub, resumeId, errorMessage) {
  const now = new Date().toISOString();
  // Persist machine-readable terminal state plus human-readable failure reason.
  const result = await docClient.send(
    new UpdateCommand({
      TableName: getTableName(),
      Key: { userSub, resumeId },
...
async function listByUser(userSub) {
  // Query by partition key returns this user's records only.
  const result = await docClient.send(
    new QueryCommand({
      TableName: getTableName(),
  ...
  return (result.Items ?? []).map((item) => ({
    // Return API-facing projection (metadata + extraction result status payload).
