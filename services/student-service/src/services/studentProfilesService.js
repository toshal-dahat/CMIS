...
function getTableName() {
  // Read env lazily so misconfiguration fails at first real DB usage.
  if (!TABLE_NAME) {
...
async function getByUserId(userId) {
  // Primary-key lookup (fast path for most authenticated profile reads).
...
async function exists(userId) {
  // Thin helper used by flows that only need presence checks.
...
function normalizeEducationEntryForMerge(e) {
  // Standardize types/nullability so merge comparisons are deterministic.
  return {
 ...
 * Profile Auto-Fill bounty behavior:
 * - backend writes extracted GPA/education/skills into profile-level fields
 * - frontend can read these fields via GET /profiles/me and pre-populate editable inputs
...
async function create(userId, profile) {
  ...
  // Initialize full profile shape so downstream reads don't require undefined checks.
  const item = {
    userId,
    name: profile.name,
...
async function update(userId, profile) {
  ...
  for (const key of allowed) {
    if (profile[key] !== undefined) {
      // Build a dynamic SET expression from whitelisted mutable fields only.
      const nameKey = `#${key}`;
      const valueKey = `:${key}`;
  ...
  if (updates.length === 0) {
    // No-op updates return current state without writing updatedAt.
...
async function remove(userId) {
  // Hard delete by PK; callers handle any upstream authorization checks.
  await docClient.send(
    new DeleteCommand({
      TableName: getTableName(),
      Key: { userId },
...
async function listAllExceptUserId(excludeUserId) {
  // Scan + server-side filter: acceptable for current table size/access pattern.
  const result = await docClient.send(
    new ScanCommand({
      TableName: getTableName(),
...
async function listAllExceptUserIdByRole(excludeUserId, role) {
  ...
  // Reuse exclude-user scan, then normalize role comparison in memory.
  const result = await docClient.send(
    new ScanCommand({
      TableName: getTableName(),
      FilterExpression: "userId <> :excludeUserId",
