...
async function emailCsvHasCompanyDomain(emailCsv) {
  const parts = (emailCsv ?? "")
    .split(",")
    .map((p) => p.trim().toLowerCase())
    .filter(Boolean);
  ...
  let companyDomains = new Set();
  try {
    companyDomains = await getCompanyDomains();
  } catch (e) {
    // If the company list API is unreachable, we treat it as non-company.
    return false;
  }

  return parts.some((email) => {
    const domain = extractDomain(email);
    return domain && companyDomains.has(domain);
  });
}

/**
 * Sync Cognito groups for users saving/updating their profile.
 *
 * Rules:
 * - If email CSV contains `@tamu.edu`: treat user as TAMU (students/alumni behavior).
 * - If email CSV contains a domain from the companies list: ensure `investors` group.
 * - If profileRole is `FORMER_STUDENT`: ensure `alumni` group (and optionally `investors`).
 *
 * Note: this function is still intentionally limited to users whose graduation month
 * is already reached when profileRole doesn't provide an explicit target.
 */
async function syncGraduatedGroupsForProfile(claims, emailCsv, gradDate, profileRole) {
  ...
  const hasCompanyDomain = await emailCsvHasCompanyDomain(emailCsv);

  async function ensureManagedGroup(desiredGroup) {
    if (!existingGroups.includes(desiredGroup)) {
      await addUserToGroup(username, desiredGroup);
    }
    const toRemove = [GROUP_STUDENTS, GROUP_INVESTORS, GROUP_FRIENDS].filter((g) => g !== desiredGroup);
  ...
  if (profileRole === "STUDENT") {
    // TAMU future grads are students; company domains are investors; otherwise friends.
    if (hasTamu) {
      await ensureManagedGroup(GROUP_STUDENTS);
    } else if (hasCompanyDomain) {
      await ensureManagedGroup(GROUP_INVESTORS);
    } else {
      await ensureManagedGroup(GROUP_FRIENDS);
    }
  ...
  if (profileRole === "FORMER_STUDENT") {
    // Former student => always alumni, plus investors if company domain matches.
    if (!existingGroups.includes(GROUP_ALUMNI)) {
    ...
    if (hasCompanyDomain) {
      ...
      // Default alumni experience for non-company is friends (not students).
      await ensureManagedGroup(GROUP_FRIENDS);
    }
    return;
  }
  ...
  if (profileRole === "FRIEND") {
    // Friend (non-former) => alumni removed, but keep investors if company domain matches.
    if (existingGroups.includes(GROUP_ALUMNI)) {
      try {
        await removeUserFromGroup(username, GROUP_ALUMNI);
      } catch (_) {
  ...
  if (hasTamu) {
    ...
    // Company domain => investors (and non-alumni by default).
    await ensureManagedGroup(GROUP_INVESTORS);
    if (existingGroups.includes(GROUP_ALUMNI)) {
      try {
        await removeUserFromGroup(username, GROUP_ALUMNI);
