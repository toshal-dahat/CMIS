...
| `smsOptIn` | boolean | No | Whether user opts in to SMS event reminders. If `true`, `phoneNumber` is required. |
| `phoneNumber` | string \| `null` | Conditional | Phone number used for SMS reminders. Required when `smsOptIn` is `true`; cleared when opting out. |
...
| `smsOptIn` | boolean | No | Whether user opts in to SMS event reminders. |
| `phoneNumber` | string \| `null` | No | Phone number used for SMS reminders; required when `smsOptIn` is `true`. |
...
### 14. Trigger Event SMS Reminder Now (Admin)

Manual endpoint to trigger the same reminder workflow used by the scheduler. Useful for testing without waiting 1 hour.

| Field | Value |
|-------|-------|
| **Method** | `POST` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/events/{eventId}/sms-reminder/trigger` |
| **Auth** | Required (Admin group) |
| **Body** | Optional (`{}`) |
