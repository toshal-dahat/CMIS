...
| Endpoint | Full URL |
|----------|----------|
| Profile Exists | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/users/me/profile-exists` |
| List Profiles (Students Connect) | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/profiles` |
| Profiles (CRUD) | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/profiles` |
| My Profile | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/profiles/me` |
| Resume Upload URL | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/upload-url` |
| Resume Complete | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/complete` |
| My Resumes | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/me` |
| Download URL | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/{resumeId}/download-url` |
...
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/users/me/profile-exists` |
| **Auth** | Required |
| **Request Body** | None |
| **Query Params** | None |
...
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/profiles` |
| **Auth** | Required |
| **Request Body** | None |
| **Query Params** | None |
...
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/profiles/me` |
| **Auth** | Required |
| **Request Body** | None |
| **Query Params** | None |
...
| `mentorshipInterested` | boolean | Yes (non-admin) | Whether the user wants to participate in the mentorship program. Admins may omit; defaults to `false`. |
| `mentorship` | `"mentee"` \| `"mentor"` \| `null` | Conditional | Required when `mentorshipInterested` is `true`; must be `null` when not interested. |
| `mentorCapacity` | number \| `null` | Conditional | When `mentorship` is `"mentor"`, must be an integer from 1 to 10. Otherwise `null`. |

When `mentorship` is `"mentee"`, at least one email in `email` must end with `@tamu.edu` (current student path). Cognito student grouping and existing role rules are unchanged.
...
| `role` | string | No | Role for this profile (e.g. `"student"`) |
| `mentorshipInterested` | boolean | No | Mentorship program participation (see create profile). |
| `mentorship` | `"mentee"` \| `"mentor"` \| `null` | No | Same rules as create when updating participation. |
| `mentorCapacity` | number \| `null` | No | Same rules as create when role is mentor. |
...
|-------|-------|
| **Method** | `POST` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/upload-url` |
| **Auth** | Required |
| **Request Body** | JSON |
...
|-------|-------|
| **Method** | `POST` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/complete` |
| **Auth** | Required |
...
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/me` |
| **Auth** | Required |
...
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/resumes/{resumeId}/download-url` |
| **Auth** | Required |
...
| GET | `/student/api/profiles` | List student profiles (Students Connect) |
...
Generates a unique signed event check-in token and returns a QR code image as a data URL.

| Field | Value |
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/events/{eventId}/qr` |
...
```json
{
  "eventId": "a9c0c2b8-8ce2-4f39-b8f6-9329de4130a1",
  "eventTitle": "Spring Career Mixer",
  "signedEventCode": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "qrCodeDataUrl": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAA...",
  "stablePerEvent": true,
  "generatedAt": "2026-04-14T10:18:22.294Z"
}
```

### 12. Check In Attendee (Admin Scanner)

Decodes/scans attendee payload, validates RSVP status, then marks attendance directly in `EventRsvps`.

| Field | Value |
|-------|-------|
| **Method** | `POST` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/api/events/{eventId}/check-in` |
| **Auth** | Required (Admin group) |
| **Body** | `{ "userId": "..." }` OR `{ "scannedText": "..." }` |

#### Sample Request

```json
{
  "scannedText": "7b2f742b-8f50-4934-8e4a-ec56ac4e1ec0"
}
```
...
```json
{
  "status": "CHECKED_IN",
  "eventId": "a9c0c2b8-8ce2-4f39-b8f6-9329de4130a1",
  "userId": "7b2f742b-8f50-4934-8e4a-ec56ac4e1ec0",
  "checkedInAt": "2026-04-14T10:22:11.404Z",
...
Returns a mobile-friendly browser scanner UI that uses the camera and calls the check-in endpoint.

| Field | Value |
|-------|-------|
| **Method** | `GET` |
| **URL** | `https://trj0i1jfd6.execute-api.us-east-1.amazonaws.com/student/qr-scanner` |
| **Auth** | Required |
