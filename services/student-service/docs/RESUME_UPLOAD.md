...
Frontend                    API (Lambda)                    S3
   ...
   |------------------------------------------------------->|
   |                             |                           |
   | 3. POST /resumes/complete   |                           |
   |    { resumeId }             |                           |
   |--------------------------->| HeadObject (verify)       |
   |                             |------------------------->|
   |                             |<-------------------------|
   |                             | Update DynamoDB (UPLOADED)|
...
```javascript
await fetch(`${API_BASE}/api/resumes/complete`, {
  method: "POST",
  headers: {
    Authorization: `Bearer ${idToken}`,
    "Content-Type": "application/json",
...
```bash
curl -X POST "https://2gzy1e8qga.execute-api.us-east-1.amazonaws.com/dev/api/resumes/complete" \
  -H "Authorization: Bearer YOUR_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"resumeId":"a1b2c3d4-..."}'
```
