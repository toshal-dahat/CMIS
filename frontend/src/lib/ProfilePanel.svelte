<script lang="ts">
  ...
  function syncFormFromProfile() {
    ...
    smsPhoneNumber = typeof p.phoneNumber === 'string' ? p.phoneNumber : '';
  }
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    if (!isAdminRole && smsOptIn && !smsPhoneNumber.trim()) {
      saveError = 'Please enter a phone number when opting in to SMS reminders.';
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        smsOptIn: isAdminRole ? undefined : smsOptIn,
        phoneNumber: isAdminRole ? undefined : (smsOptIn ? smsPhoneNumber.trim() : null),
        ...(isResumeRole
          ? {
      ...
      <form class="profile-form" onsubmit={handleSubmit}>
        ...
        {#if panelRole !== 'admin'}
          ...
          <fieldset class="mentorship-fieldset">
            ...
            <p class="field-hint">Get a text reminder about 1 hour before your confirmed events.</p>
            <label class="mentorship-radio-label" for="panel-smsOptIn">
              <input id="panel-smsOptIn" type="checkbox" bind:checked={smsOptIn} />
              Opt-in to SMS
            </label>
            {#if smsOptIn}
              <div class="field" style="margin-top: 0.75rem;">
                <label for="panel-smsPhoneNumber">Phone number <span class="required-mark">*</span></label>
                <input
                  id="panel-smsPhoneNumber"
                  type="tel"
                  bind:value={smsPhoneNumber}
                  placeholder="+1XXXXXXXXXX"
                  autocomplete="tel"
                />
                <p class="field-hint">Use E.164 format if possible (example: +1XXXXXXXXXX).</p>
              </div>
            {/if}
          </fieldset>
          {#if panelRole === 'friend' || panelRole === 'investor'}
