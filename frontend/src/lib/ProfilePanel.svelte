<script lang="ts">
  ...
  function syncFormFromProfile() {
    ...
    reminderOptIn = p.reminderOptIn !== false;
  }
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        reminderOptIn: isAdminRole ? undefined : reminderOptIn,
        ...(isResumeRole
          ? {
    ...
    <div class="panel" onclick={(e) => e.stopPropagation()}>
      ...
      <form class="profile-form" onsubmit={handleSubmit}>
        ...
        {#if panelRole !== 'admin'}
          ...
          <div class="field">
            <label for="panel-reminderOptIn">Event reminder emails</label>
            <div style="display:flex;align-items:center;gap:0.5rem;">
              <input
                id="panel-reminderOptIn"
                type="checkbox"
                bind:checked={reminderOptIn}
                style="width:auto;"
              />
              <span class="field-hint" style="font-size:0.9rem;">Opt-in to reminders (1 hour before your RSVPed events).</span>
            </div>
          </div>
          {#if panelRole === 'friend' || panelRole === 'investor'}
...
<style>
  ...
  .actions {
    ...
    margin-top: 0.5rem;
    position: sticky;
    /* Offset panel padding so sticky bar sits at true panel bottom. */
    bottom: -1.75rem;
    z-index: 2;
    margin-left: -2rem;
    margin-right: -2rem;
