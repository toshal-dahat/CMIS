<script lang="ts">
  ...
  // If profile data arrives after panel render, hydrate only empty fields.
  // This avoids wiping user edits while still fixing blank form on login/open.
  $effect(() => {
    if (!open) return;
    const p = $profile;
    ...
    if (!selectedProfileRole && p.role) selectedProfileRole = p.role;
    if (!resumeS3Key && p.resumeS3Key) resumeS3Key = p.resumeS3Key;
  });
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    if (!result.ok && result.error?.toLowerCase().includes('profile not found')) {
