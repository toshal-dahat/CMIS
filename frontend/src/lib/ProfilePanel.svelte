  ...
  <div class="backdrop" role="dialog" aria-modal="true" aria-labelledby="profile-panel-title" onclick={handleBackdropClick}>
    <div class="panel" onclick={(e) => e.stopPropagation()}>
      ...
      <form class="profile-form" onsubmit={handleSubmit}>
        ...
        {#if panelRole !== 'admin'}
          <div class="field">
            <label for="panel-reminderOptIn">Email reminders</label>
            <div style="display:flex;align-items:center;gap:0.5rem;">
              <input id="panel-reminderOptIn" type="checkbox" bind:checked={reminderOptIn} style="width:auto;" />
              <span class="field-hint" style="font-size:0.9rem;">Opt-in to reminders (1 hour before events)</span>
            </div>
          </div>

          <div class="field">
            <label for="panel-phoneNumber">Phone number <span class="hint">(optional, for SMS reminders)</span></label>
            <input
              id="panel-phoneNumber"
              type="tel"
...
<style>
  ...
  /* Keep primary form actions visible while long profile sections scroll. */
  .actions {
    display: flex;
    gap: 0.75rem;
    margin-top: 0.5rem;
    position: sticky;
    /*
     * The panel uses 1.75rem bottom padding and 2rem horizontal padding.
     * Negative bottom + side margins let this sticky row align to the panel
     * edge instead of floating above the padding area.
     */
    bottom: -1.75rem;
    z-index: 2;
    margin-left: -2rem;
  ...
  /*
   * Sign-out is intentionally styled as a danger action to match the
   * profile-menu sign-out button and make the destructive/auth action obvious.
   */
  .btn-signout {
    padding: 0.5rem 1rem;
    font-size: 0.9rem;
  ...
  /* Slightly deeper red + tint on hover for clear affordance. */
  .btn-signout:hover {
    color: #8d2018;
    background: #fde8e8;
    border-color: #e79a9a;
