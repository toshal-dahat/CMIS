<script lang="ts">
  ...
  // SMS fields are explicit opt-in and E.164 formatted phone storage.
  ...
  function normalizePhoneE164Input(value: string): string {
    // Keep user input trimmed; backend enforces full E.164 validation.
    return (value ?? '').trim();
  }
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    if (smsOptIn) {
      const normalizedPhone = normalizePhoneE164Input(phoneE164);
      if (!/^\+[1-9]\d{6,14}$/.test(normalizedPhone)) {
        saveError = 'Phone number must be E.164 format (example: +19795551234).';
        return;
      }
    ...
    try {
      ...
      const payload = {
        ...
        smsOptIn,
        phoneE164: smsOptIn ? normalizePhoneE164Input(phoneE164) : null,
        ...(isResumeRole
          ? {
    ...
    <div class="panel" onclick={(e) => e.stopPropagation()}>
      ...
      <form class="profile-form" onsubmit={handleSubmit}>
        ...
        {#if panelRole !== 'admin'}
          ...
          {#if panelRole === 'student'}
            <div class="field">
              <label for="panel-smsOptIn">Opt-in to SMS</label>
              <div style="display:flex;align-items:center;gap:0.5rem;">
                <input id="panel-smsOptIn" type="checkbox" bind:checked={smsOptIn} style="width:auto;" />
                <span class="field-hint" style="font-size:0.9rem;">Receive event reminders 1 hour before your RSVP'd events.</span>
              </div>
            </div>
            {#if smsOptIn}
              <div class="field">
                <label for="panel-phoneE164">Phone Number (E.164) <span class="required-mark">*</span></label>
                <input
                  id="panel-phoneE164"
                  type="tel"
                  bind:value={phoneE164}
                  placeholder="+19795551234"
                />
                <p class="field-hint">Use country code and no spaces, for example <code>+19795551234</code>.</p>
              </div>
            {/if}
          {/if}
          {#if panelRole === 'friend' || panelRole === 'investor'}
