<script lang="ts">
  ...
  import ProfileMatchChecklist from './ProfileMatchChecklist.svelte';
  ...
  let mentorshipInterest = $state<'' | 'yes' | 'no'>('');
  let mentorshipKind = $state<'' | 'mentee' | 'mentor'>('');
  let mentorCapacityNum = $state(5);
  ...
  let showMentorModal = $state(false);
  let mentorModalPrompted = $state(false);
  ...
  let mentorshipGoalsText = $state('');
  let studentGoalsText = $state('');
  ...
  function syncFormFromProfile() {
    ...
    if (p.mentorshipInterested === true) {
      mentorshipInterest = 'yes';
      mentorshipKind = p.mentorship === 'mentee' || p.mentorship === 'mentor' ? p.mentorship : '';
      mentorCapacityNum = typeof p.mentorCapacity === 'number' ? p.mentorCapacity : 5;
      ...
      mentorshipGoalsText =
        p.mentorship === 'mentee' && typeof p.mentorshipGoals === 'string' ? p.mentorshipGoals : '';
      studentGoalsText =
        p.mentorship === 'mentee' && typeof p.studentGoals === 'string' ? p.studentGoals : '';
      mentorBioText =
        p.mentorship === 'mentor' && typeof p.mentorBio === 'string' ? p.mentorBio : '';
    } else if (p.mentorshipInterested === false) {
  ...
  $effect(() => {
    if (mentorshipInterest === 'yes' && mentorshipKind === 'mentor' && !mentorModalPrompted) {
      showMentorModal = true;
      mentorModalPrompted = true;
    }
    if (mentorshipKind !== 'mentor') {
      mentorModalPrompted = false;
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    try {
      ...
      if (!isAdminRole) {
        if (mentorshipInterest === '') {
          saveError = 'Please indicate whether you are interested in the mentorship program.';
          return;
        }
        if (mentorshipInterest === 'yes') {
          if (mentorshipKind !== 'mentee' && mentorshipKind !== 'mentor') {
            saveError = 'Please select mentee or mentor for the mentorship program.';
            return;
          }
          if (mentorshipKind === 'mentor') {
            ...
            if (!mentorProfileReady()) {
              saveError =
                'Please complete the mentor details popup (skills, industries, company, and valid years of experience).';
              showMentorModal = true;
              return;
            }
          }
          if (mentorshipKind === 'mentee' && !csvHasTamu(finalEmailCsv)) {
            saveError =
              'Mentee participation requires at least one TAMU email (@tamu.edu) on your profile.';
            return;
          }
        }
      }
      ...
      const payload = {
        ...
        role: explicitRole,
        mentorshipInterested: isAdminRole ? false : mentorshipInterest === 'yes',
        mentorship:
          !isAdminRole && mentorshipInterest === 'yes'
            ? mentorshipKind === 'mentee' || mentorshipKind === 'mentor'
              ? mentorshipKind
              : null
            : null,
        mentorCapacity:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
            ? mentorCapacityNum
            : null,
        mentorSkills:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
            ? parseCsvTags(mentorSkillsInput)
            : [],
        mentorIndustries:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
        ...
        mentorCompany:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
            ? mentorCompany.trim() || null
            : null,
        mentorJobTitle:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
            ? mentorJobTitle.trim() || null
            : null,
        mentorYearsExperience:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
            ? String(mentorYearsExperienceInput ?? '').trim()
              ? Number(String(mentorYearsExperienceInput ?? '').trim())
              : null
            : null,
        mentorshipGoals:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentee'
            ? mentorshipGoalsText.trim() || null
            : null,
        studentGoals:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentee'
            ? studentGoalsText.trim() || null
            : null,
        mentorBio:
          !isAdminRole && mentorshipInterest === 'yes' && mentorshipKind === 'mentor'
            ? mentorBioText.trim() || null
            : null,
        ...(isResumeRole
          ? {
      ...
      <form class="profile-form" onsubmit={handleSubmit}>
        ...
        {#if panelRole !== 'admin'}
          ...
          <fieldset class="mentorship-fieldset">
            <legend>Mentorship program <span class="required-mark">*</span></legend>
            <p class="field-hint">
              You can change this anytime. If you opt in, choose mentee or mentor below.
            </p>
            <div class="mentorship-radios">
              <label class="mentorship-radio-label">
                <input type="radio" name="panel-mentorship-interest" value="no" bind:group={mentorshipInterest} />
                Not interested
              </label>
              <label class="mentorship-radio-label">
                <input
                  type="radio"
                  name="panel-mentorship-interest"
                  value="yes"
                  bind:group={mentorshipInterest}
                />
            ...
            {#if mentorshipInterest === 'yes'}
              <div class="mentorship-followup">
                <p class="field-hint" style="margin-top: 0.75rem;">I would like to participate as:</p>
                <div class="mentorship-radios">
                  <label class="mentorship-radio-label">
                    <input type="radio" name="panel-mentorship-kind" value="mentee" bind:group={mentorshipKind} />
                    Mentee
                  </label>
                  <label class="mentorship-radio-label">
                    <input type="radio" name="panel-mentorship-kind" value="mentor" bind:group={mentorshipKind} />
                    Mentor
                  </label>
                </div>
                {#if mentorshipKind === 'mentee'}
                  <div class="field" style="margin-top: 0.75rem;">
                    <label for="panel-mentorship-goals"
                      >Career goals <span class="hint">(recommended, especially without a resume)</span></label
                    >
                    <textarea
                      id="panel-mentorship-goals"
                      rows="3"
                      bind:value={mentorshipGoalsText}
                      placeholder="e.g. Internship in cloud security; strengthen Python and CI/CD; learn how teams ship to production."
                      maxlength="2000"
                    ></textarea>
                    <p class="field-hint">
                      Matching blends this text with your skills, major, and education. Specific goals beat a one-line “open to anything.”
                    </p>
                  </div>
                  <div class="field" style="margin-top: 0.75rem;">
                    <label for="panel-student-goals"
                      >Mentorship goals <span class="hint">(Describe what you want to achieve with a mentor)</span></label
                    >
                    <textarea
                      id="panel-student-goals"
                      rows="3"
                      bind:value={studentGoalsText}
                      placeholder="e.g. I want resume reviews, mock interviews, and advice on breaking into a PM role."
                      maxlength="2000"
                    ></textarea>
                  </div>
                {/if}
                {#if mentorshipKind === 'mentor'}
                  ...
                  <button type="button" class="btn-secondary" style="margin-top:0.6rem;" onclick={() => (showMentorModal = true)}>
                    Configure mentor details
                  </button>
                {/if}
              </div>
            {/if}
          ...
          <ProfileMatchChecklist
            active={mentorshipInterest === 'yes' && (mentorshipKind === 'mentee' || mentorshipKind === 'mentor')}
            mentorshipKind={mentorshipKind}
            skillKeysCount={selectedSkillKeys.length}
            goalsTrimmed={mentorshipGoalsText}
            educationEntries={educationEntries}
            profileGpaStr={profileGpaStr}
            {major}
            {gradDate}
            {linkedinUrl}
            mentorSkillsCsv={mentorSkillsInput}
            mentorIndustriesCount={mentorIndustriesSelected.length}
            {mentorCompany}
            {mentorJobTitle}
            mentorYearsStr={mentorYearsExperienceInput}
          />
...
{#if showMentorModal}
  <div class="mentor-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="mentor-panel-modal-title">
    <div class="mentor-modal">
      <h3 id="mentor-panel-modal-title">Mentor details</h3>
      <p class="field-hint">This popup appears only for mentors. Mentees do not need to fill these fields.</p>
      <div class="field">
      ...
      <div class="field">
        <label for="panel-mentor-title">Role / job title</label>
        <input id="panel-mentor-title" type="text" bind:value={mentorJobTitle} placeholder="e.g. Senior Data Analyst" />
      </div>
      <div class="field">
      ...
      <div class="actions">
        <button type="button" class="btn-secondary" onclick={() => (showMentorModal = false)}>Done</button>
      </div>
    </div>
  </div>
{/if}
