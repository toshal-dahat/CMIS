<script lang="ts">
  ...
  import { startMentorshipMatchingRun } from '../admin-api/mentorship-matching';
  ...
  let refreshingMentorAssignments = $state(false);
  let runningAdminMatching = $state(false);
  ...
  async function showNextMenteeNow(): Promise<void> {
    ...
    await refreshMentorAssignments(true);
  }
  ...
  const isMentorshipAdmin = $derived($currentRole === 'admins' || ($profile.role || '').toLowerCase() === 'admin');
  ...
  async function refreshMentorAssignments(rebuildIfEmpty = true): Promise<boolean> {
    const loaded = await loadMentorMatches();
    if (!loaded) return false;
    if (!rebuildIfEmpty) return true;

    const hasActiveQueue = matches.some((m) => {
      const s = String(m.status || '').toUpperCase();
      return s === 'SUGGESTED' || s === 'PENDING_MENTOR';
    });
    if (hasActiveQueue || isMentorReminderActive(mentorReminderUntil)) return true;

    const built = await fetchMentorshipCandidates();
    if (!built.ok) {
      if (built.status !== 404) {
        error = built.error || 'Failed to build mentorship assignments.';
      }
      return false;
    }
    syncMentorPauseFromApi(built.data?.mentorPauseUntil);
    if (built.data?.mentorBoard) mentorBoardInfo = built.data.mentorBoard;
  ...
  async function refreshMentorMatches(): Promise<void> {
    ...
    success = '';
    loading = true;
    refreshingMentorAssignments = true;
    await refreshMentorAssignments(true);
    refreshingMentorAssignments = false;
    loading = false;
  }

  async function runAdminMatchingFromCard(): Promise<void> {
    if (runningAdminMatching) return;
    runningAdminMatching = true;
    error = '';
    success = '';
    try {
      const run = await startMentorshipMatchingRun({ resetMatches: true, runBatchMatching: true });
      const queued = String(run.status || '').toUpperCase() === 'QUEUED' || run.accepted === true;
      if (queued) {
        success = 'Admin matching run queued. Refreshing assignments as it completes…';
        for (let i = 0; i < 4; i += 1) {
          await sleep(1200);
          await refreshMentorAssignments(true);
          const hasActiveQueue = matches.some((m) => {
          ...
          if (hasActiveQueue) break;
        }
      } else {
        success = 'Admin matching run completed. Assignments refreshed.';
        await refreshMentorAssignments(true);
      }
    } catch (e: any) {
      error = e?.message || 'Failed to run admin matching.';
    } finally {
      runningAdminMatching = false;
    }
  }
    ...
    {:else if isMentor}
      ...
      <section class="panel panel-highlight animate-in-up">
        ...
        <button
          ...
          type="button"
          onclick={() => void refreshMentorMatches()}
          disabled={refreshingMentorAssignments}
        >
          {refreshingMentorAssignments ? 'Refreshing…' : 'Refresh & find next'}
        </button>
        {#if isMentorshipAdmin}
          <button
            class="btn-primary btn-inline"
            type="button"
            onclick={() => void runAdminMatchingFromCard()}
            disabled={runningAdminMatching}
          >
            {runningAdminMatching ? 'Running matching…' : 'Run matching now (admin)'}
          </button>
        {/if}
      </section>
