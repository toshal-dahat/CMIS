<script lang="ts">
  ...
  function scoreBandLabel(percent: number): string {
    if (percent >= 85) return 'Excellent fit';
  ...
  async function acceptCandidate(menteeUserId: string): Promise<void> {
    ...
    error = '';
    success = '';
    const hadAcceptedBefore = mentorAcceptedMatches.length > 0;
    acceptingMenteeUserId = menteeUserId;
    try {
      const idempotencyKey = `${menteeUserId}:${Date.now()}`;
      const res = await acceptMentorshipMatch(menteeUserId, idempotencyKey);
      if (!res.ok) {
        if (res.status === 409) {
      ...
      if (!hadAcceptedBefore) {
        const wantsMore = window.confirm(
          'You accepted your first mentee. Do you want to continue receiving more student suggestions?',
        );
        if (!wantsMore) {
          const pauseUntil = isoDateAfterDays(90);
          const pauseRes = await putMentorshipMentorPause({
            until: pauseUntil,
            clearActiveSuggestions: true,
          });
          if (pauseRes.ok) {
            syncMentorPauseFromApi(pauseRes.data?.mentorPauseUntil ?? pauseUntil);
            success += ` New suggestions paused until ${pauseUntil}. You can resume anytime from this page.`;
            await loadMentorMatches();
          } else {
            error = pauseRes.error || 'Could not pause additional suggestions after acceptance.';
          }
        } else {
          success += ' We will continue showing next suggestions one at a time.';
        }
      }
    } finally {
  ...
  <div class="inner">
    ...
    {:else if isMentor}
      ...
      {#if !mentorPausedByReminder && mentorCurrentPending}
          {@const overallScore = primaryMatchScore(mentorCurrentPending)}
          {@const overallPct = scorePercent(overallScore)}
          {@const semanticPct = scorePercent(numericScore(mentorCurrentPending.semanticScore))}
          {@const rulePct = scorePercent(numericScore(mentorCurrentPending.ruleScore))}
          <section class="panel animate-in-up">
              ...
              <div class="pending-top">
                <div class="pending-headline">
                  ...
                  <div class="score-inline">
                    <span class="score-hint">Overall match</span>
                    <span class="score-pct">{overallPct}%</span>
                    <span class="score-sep">·</span>
                    <span class="muted small">{scoreBandLabel(overallPct)}</span>
                  </div>
                  <div class="fit-bar-track fit-bar-animated">
                    <div class="fit-bar-inner" style={`width: ${overallPct}%`}></div>
                  </div>
                </div>
                <div class="score-meter-cluster">
                  <span class="fit-donut fit-donut-lg" style={`--fit-pct: ${overallPct}`}></span>
                  <div class="score-block-rich">
                    <span class="score-hint">Confidence</span>
                    <span class="score-pct-lg">{overallPct}%</span>
                  </div>
                </div>
              </div>
              <div class="score-chip-row">
                <span class="score-chip">
                  <span class="score-chip-label">Semantic</span>
                  <strong>{semanticPct}%</strong>
                </span>
                <span class="score-chip">
                  <span class="score-chip-label">Rules</span>
                  <strong>{rulePct}%</strong>
                </span>
                <span class="score-chip score-chip-accent">
                  <span class="score-chip-label">Final</span>
