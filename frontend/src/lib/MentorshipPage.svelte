...
<div class="mentorship-page">
  <div class="inner">
    ...
    {:else if isMentor}
      ...
      {#if mentorBoard && typeof mentorBoard.tier === 'string'}
        <div
          class="tier-banner animate-in-up"
          class:tier-banner-standard={(mentorBoard.tier || 'none').toLowerCase() === 'none'}
          role="status"
        >
          <span class="tier-label">{boardPartnerHeadline(mentorBoard.tier)}</span>
          <span class="tier-meta">
            {#if (mentorBoard.tier || 'none').toLowerCase() !== 'none'}
              Ranking boost ×{typeof mentorBoard.multiplier === 'number' ? mentorBoard.multiplier.toFixed(2) : '—'} when we rank
              students for you.
            {:else}
              No Gold/Silver board boost on your profile. Partner companies in the board list (or configured domains) unlock
              tiered boosts.
            {/if}
          </span>
          {#if mentorBoard.reason?.trim()}
            <span class="tier-reason muted small">{mentorBoard.reason}</span>
          {/if}
        </div>
      {/if}
