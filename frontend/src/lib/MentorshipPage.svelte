<script lang="ts">
  ...
  let acceptingMenteeUserId = $state('');
  ...
  function openCantMentorModal(row: MentorshipMatchRow): void {
    ...
    cantMentorPauseDate = todayIsoDate();
  ...
  async function confirmCantMentorPauseProgram(): Promise<void> {
    ...
    if (!pauseDay || pauseDay < todayIsoDate()) {
      error = 'Choose a date (today or later) when you want to be considered for matching again.';
      return;
    }
    error = '';
  ...
  async function acceptCandidate(menteeUserId: string): Promise<void> {
    if (acceptingMenteeUserId && acceptingMenteeUserId === menteeUserId) return;
    ...
    if (!res.ok) {
      if (res.status === 409) {
        ...
        } else if (/match row changed|accepted\/updated elsewhere|match_row_unavailable/i.test(joined)) {
          await loadMentorMatches();
          error =
            'This student pairing changed just now (for example, another accept action already completed). The list is refreshed.';
        } else {
          error =
            joined ||
            res.error ||
...
<div class="mentorship-page">
  <div class="inner">
      ...
      <div class="mentor-modal" role="dialog" aria-modal="true" aria-labelledby="cant-mentor-title">
        ...
        <div class="mentee-detail-block">
          ...
          <div class="row-actions">
            <input type="date" bind:value={cantMentorPauseDate} min={todayIsoDate()} />
    ...
    {:else if isMentor}
      ...
      {:else if !mentorPausedByReminder && mentorPendingSorted.length > 0}
        {#each mentorPendingSorted as row, ri (row.menteeUserId || String(ri))}
          <section class="panel animate-in-up">
            ...
            <article class="simple-row pending-card">
              ...
              {#if row.reasonSummary}
                <p class="muted small reason-fade">{row.reasonSummary}</p>
              {/if}
              {#if row.matchedSignals?.length}
                <p class="muted small"><strong>Signals:</strong> {row.matchedSignals.join(' · ')}</p>
              {/if}
              {@render menteeReviewDetails(row)}
              {#if row.menteeUserId}
                <div class="row-actions">
                  <button
                    class="btn-primary"
                    type="button"
                    onclick={() => acceptCandidate(row.menteeUserId!)}
                    disabled={acceptingMenteeUserId === row.menteeUserId}
                  >
                    {acceptingMenteeUserId === row.menteeUserId ? 'Accepting…' : 'Accept'}
                  </button>
                  <button class="btn-secondary" type="button" onclick={() => openCantMentorModal(row)}>Can't mentor right now</button>
                </div>
              {/if}
            </article>
          </section>
        {/each}
      {:else if !mentorPausedByReminder}
        <section class="panel animate-in-up">
