<script lang="ts">
  ...
  /** Friend/investor: checkbox to participate as a mentor (students are mentees by default server-side). */
  let mentorshipParticipation = $state(false);
  ...
  function hydrateFromProfile() {
    ...
    if (signupRole === 'friend' || signupRole === 'investor') {
      const isMentorProfile =
        p.mentorshipInterested === true && p.mentorship === 'mentor';
      mentorshipParticipation = isMentorProfile;
  ...
  function mentorshipRoleLabel(): string {
    ...
    if (signupRole === 'friend' || signupRole === 'investor') {
      return mentorshipParticipation ? 'Mentor' : 'Not participating';
    }
  ...
  const handleSubmit = async (event: SubmitEvent) => {
    ...
    try {
      ...
      const interestedMentor = !isAdminRole && isFriendOrInvestor && mentorshipParticipation;
      ...
      const result = await createProfile({
        ...
        mentorshipInterested:
          isAdminRole ? false : studentMentorshipDefaults ? true : interestedMentor,
      ...
      {#if signupRole === 'friend' || signupRole === 'investor'}
        <fieldset class="mentorship-fieldset">
          ...
          <div class="field" style="margin-top: 0.5rem;">
            <label class="mentorship-checkbox-label" style="display:flex;align-items:flex-start;gap:0.5rem;cursor:pointer;">
              <input type="checkbox" bind:checked={mentorshipParticipation} style="width:auto;margin-top:0.2rem;" />
              <span>Interested in the mentorship program</span>
            </label>
          </div>
          {#if mentorshipParticipation}
