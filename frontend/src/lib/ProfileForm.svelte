<script lang="ts">
  ...
  const handleSubmit = async (event: SubmitEvent) => {
    ...
    if (!isAdminRole && smsOptIn && !smsPhoneNumber.trim()) {
      submitError = 'Please enter a phone number when opting in to SMS reminders.';
      return;
    }
  ...
  <form class="profile-form" novalidate onsubmit={handleSubmit}>
    ...
    {#if signupRole !== 'admin'}
      ...
      <fieldset class="mentorship-fieldset">
        ...
        <label class="mentorship-radio-label" for="smsOptIn">
          <input id="smsOptIn" type="checkbox" bind:checked={smsOptIn} />
          Opt-in to SMS
        </label>
        {#if smsOptIn}
          <div class="field" style="margin-top: 0.75rem;">
            <label for="smsPhoneNumber">Phone number <span class="required-mark">*</span></label>
            <input
              id="smsPhoneNumber"
              type="tel"
