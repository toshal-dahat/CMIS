  ...
  const handleSubmit = async (event: SubmitEvent) => {
    ...
    if (smsOptIn) {
      ...
      if (!/^\+[1-9]\d{6,14}$/.test(normalizedPhone)) {
        submitError = 'Phone number must be E.164 format (example: +19795551234).';
        return;
      }
...
<section class="profile-form-container">
  ...
  <form class="profile-form" novalidate onsubmit={handleSubmit}>
    ...
    {#if signupRole !== 'admin'}
      ...
      {#if signupRole === 'student'}
        <div class="field">
          <label for="smsOptIn">Opt-in to SMS</label>
          <div style="display:flex;align-items:center;gap:0.5rem;">
            <input id="smsOptIn" type="checkbox" bind:checked={smsOptIn} style="width:auto;" />
            <span class="field-hint" style="font-size:0.9rem;">Receive event reminders 1 hour before your RSVP'd events.</span>
          </div>
        </div>
        {#if smsOptIn}
          <div class="field">
            <label for="phoneE164">Phone Number (E.164) <span class="required-mark">*</span></label>
            <input
              id="phoneE164"
              type="tel"
