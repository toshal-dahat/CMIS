...
/**
 * Computes the Velvet Rope status for a user and a specific event.
 *
 * This function determines if the user is currently allowed to RSVP based on:
 * 1. Has the event already started?
 * 2. Is the RSVP deadline past?
 * 3. Is the user exempt (e.g., they created the event or have an explicit bypass)?
 * 4. What is the user's tier and early access hours? (Using global config or event-specific overrides).
 *
 * It calculates `myUnlockTs` (the exact timestamp the user can RSVP) and returns
 * a countdown message if they are not yet eligible.
 *
 * @param eventDate - ISO string of the event start time.
 * @param eventCreatedBy - The ID of the user who created the event.
 * @param currentUserId - The ID of the current authenticated user.
 * @param state - The global VelvetRopeStore state (contains user's tier, early access config).
 * @param eventBypass - If true, bypasses the velvet rope entirely for this event.
 * @param eventTierOverrides - Optional event-specific override hours mapped by tier ID.
 * @param eventDeadline - Optional ISO string deadline for RSVPs.
 * @returns {VelvetRopeStatus} The computed status, including if they can RSVP right now and a formatted countdown message if they cannot.
 */
export function computeVelvetRopeStatus(
    ...
    if (state.userTierId && state.userTierId !== 'standard') {
        const matchingTier = state.tiers.find(t => (t.tierId || '').toLowerCase() === (state.userTierId || '').toLowerCase());
        const name = (matchingTier && matchingTier.name) ? matchingTier.name : (state.userTierId.charAt(0).toUpperCase() + state.userTierId.slice(1));
