<script lang="ts">
  ...
  async function handleTeamTrigger(panel: CompetitionPanel, team: Team, refresh = false) {
    ...
    try {
      const result = await triggerSynthesis(panel.competition.competitionId, team.teamId, refresh);
      const cacheNote = result.cached ? " (Retrieved from cache)" : "";
      panel.teamSynthesis[team.teamId] = {
        loading: false,
        status: `✅ Synthesized ${result.judgeCount === -1 ? 'assigned' : result.judgeCount} judge comment(s) for ${team.teamName}.${cacheNote}`,
        error: "",
      };
    } catch (err: unknown) {
      panel.teamSynthesis[team.teamId] = {
                          ...
                          <tr>
                            ...
                            <td>
                              <button
                                ...
                                onclick={() => handleTeamTrigger(panel, team, !!synth?.status)}
                              >
                                {synth?.loading ? "Synthesizing…" : (synth?.status ? "Regenerate AI Synthesis" : "Trigger AI Synthesis")}
                              </button>
                              {#if synth?.status}
