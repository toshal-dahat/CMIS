<script lang="ts">
  ...
  type TopAttendeeMode = "checkin" | "rsvp" | "both";
  ...
  async function loadAnalytics() {
    ...
    try {
      ...
      const perEventRsvps = await Promise.all(
        eventIds.map((eventId) => fetchWithRetry(() => getEventRsvps(eventId))),
      );
      const rsvpRows = perEventRsvps.flat();
  ...
  function topAttendeesEmptyMessage(mode: TopAttendeeMode): string {
    ...
    if (mode === "both") return "No student RSVP or check-in data in this scope yet.";
      ...
      <div class="panel">
        <div class="panel-head">
          ...
          >
            ...
            <option value="both">Both (check-ins, then RSVPs)</option>
          </select>
        </div>
...
{#if listModal === "top"}
  ...
  <div class="modal-backdrop" onclick={(e) => e.target === e.currentTarget && closeListModal()} role="presentation">
    <div class="modal-dialog" role="dialog" aria-modal="true" aria-labelledby="modal-top-title">
      ...
      <p class="muted modal-sub">Ranked by {topAttendeeMode === "checkin" ? "check-ins" : topAttendeeMode === "rsvp" ? "RSVPs" : "check-ins, then RSVPs"} · STUDENT role only</p>
      <ul class="modal-list">
