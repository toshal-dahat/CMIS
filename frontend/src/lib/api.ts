...
function mapEducationFromApi(raw: unknown): EducationRow[] {
  if (!Array.isArray(raw)) return [];
  return raw.map((e) => {
    const o = e as Record<string, unknown>;
    const g = o.gpa;
    const gpaStr = g == null || g === '' ? '' : typeof g === 'number' ? String(g) : String(g);
    return {
      institution: String(o.institution ?? ''),
      degreeDiploma: String(o.degreeDiploma ?? o.degree ?? ''),
      specialization: String(o.specialization ?? o.field ?? ''),
      gpa: gpaStr,
      dates: String(o.dates ?? ''),
      details: String(o.details ?? ''),
    };
  });
}

function mapExperienceFromApi(raw: unknown): ExperienceRow[] {
  if (!Array.isArray(raw)) return [];
  return raw.map((x) => {
    const o = x as Record<string, unknown>;
    const h = o.highlights;
    const highlights = Array.isArray(h) ? (h as string[]).join('\n') : String(h ?? '');
    return {
...
function mapEducationToApi(rows: EducationRow[] | undefined): unknown[] | undefined {
  if (!rows?.length) return undefined;
  return rows
    .filter(
      (r) =>
        r.institution?.trim() ||
        r.degreeDiploma?.trim() ||
    ...
    .map((r) => {
      ...
      if (g) {
        ...
        gpa = Number.isFinite(n) ? n : g;
      }
      return {
        institution: r.institution?.trim() ?? '',
        degreeDiploma: r.degreeDiploma?.trim() ?? '',
...
function toProfileFromApi(data: Record<string, unknown>): Profile {
  ...
  return {
    ...
    gpa,
    education: mapEducationFromApi(data.education),
    skillIds: Array.isArray(data.skillIds) ? (data.skillIds as string[]) : undefined,
    experience: mapExperienceFromApi(data.experience),
    resumeExtra:
      data.resumeExtra && typeof data.resumeExtra === 'object'
        ? (data.resumeExtra as Profile['resumeExtra'])
        : undefined,
  };
}
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}${STUDENT_API_PREFIX}/profiles`, {
      ...
      body: JSON.stringify({
        ...
        gpa: body.gpa,
        education: mapEducationToApi(body.education),
        skillIds: body.skillIds,
        experience: body.experience?.map((x) => ({
          company: x.company,
          title: x.title,
          dates: x.dates,
...
export async function updateProfile(body: UpdateProfileBody): Promise<ApiResult> {
  try {
    ...
    if (body.education !== undefined) {
      payload.education =
        body.education.length === 0 ? [] : mapEducationToApi(body.education) ?? [];
    }
    if (body.skillIds !== undefined) payload.skillIds = body.skillIds;
    if (body.experience !== undefined) {
      payload.experience = body.experience.map((x) => ({
        company: x.company,
        title: x.title,
        dates: x.dates,
        highlights: x.highlights
    ...
    if (body.resumeExtra !== undefined) payload.resumeExtra = body.resumeExtra;
...
/** Canonical skills from DynamoDB (for profile skill picker). */
export async function fetchMasterSkills(): Promise<{ skillId: string; canonicalName: string }[]> {
  try {
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_BASE}${STUDENT_API_PREFIX}/master-skills`, { headers });
    if (!res.ok) return [];
    const data = (await res.json()) as { skills?: { skillId: string; canonicalName: string }[] };
