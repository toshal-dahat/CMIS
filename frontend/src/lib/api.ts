...
export async function fetchMentorshipMenteeMatches(): Promise<ApiResult<MentorshipMenteeStatusPayload>> {
  try {
    ...
    const parsedMatchesRaw = Array.isArray(data.matches)
      ? (data.matches as unknown[]).filter((m): m is MentorshipMatchRow => !!m && typeof m === 'object')
      : [];
    const parsedMatches = parsedMatchesRaw.map((m) => {
      const parsed = parseMentorBoardPayload((m as unknown as Record<string, unknown>).mentorBoard);
      return parsed ? { ...m, mentorBoard: parsed } : m;
    });
    ...
    const matchedMentorRaw =
      data.matchedMentor && typeof data.matchedMentor === 'object'
        ...
        : null;
    const matchedMentorFromField = matchedMentorRaw
      ? (() => {
      ...
      : null;
    const matchedMentor = parsedMatches[0] ?? matchedMentorFromField ?? null;
    const matches = parsedMatches.length > 0 ? parsedMatches : matchedMentor ? [matchedMentor] : [];
    const hasMatchedMentor = matches.length > 0;
    const state: 'MATCHING_IN_PROGRESS' | 'MATCHED' =
      stateRaw === 'MATCHED' || hasMatchedMentor ? 'MATCHED' : 'MATCHING_IN_PROGRESS';
    ...
    const payload: MentorshipMenteeStatusPayload = {
      state,
      isMatching: typeof data.isMatching === 'boolean' ? data.isMatching : !hasMatchedMentor,
      matchedMentor,
...
export async function acceptMentorshipMatch(menteeUserId: string, idempotencyKey?: string): Promise<
  ApiResult<{
...
> {
  try {
    ...
    const res = await fetch(`${EXTERNAL_API_BASE}/mentorship/matches/${encodeURIComponent(menteeUserId)}/accept`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ idempotencyKey: (idempotencyKey || '').trim() || undefined }),
    });
    const data = (await res.json().catch(() => ({}))) as Record<string, unknown>;
    if (!res.ok) {
