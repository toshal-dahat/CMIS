...
export async function putMentorshipMentorPause(body: {
  until?: string;
  clear?: boolean;
}): Promise<ApiResult<{ mentorPauseUntil?: string | null; mentorPaused?: boolean; cleared?: boolean }>> {
  try {
    const headers = await getAuthHeaders();
...
/** Row from GET /mentorship/suggested-mentors (mentee-only; includes scores for transparency). */
export interface MentorshipSuggestedMentorRow {
  mentorUserId?: string;
  mentorName?: string;
...
export async function fetchMentorshipSuggestedMentors(): Promise<
  ApiResult<{ suggestions: MentorshipSuggestedMentorRow[]; totalSuggestions: number }>
> {
  try {
    const headers = await getAuthHeaders();
    ...
    if (!res.ok) {
      return {
        ok: false,
        error: externalApiError(data, 'Failed to load suggested mentors', res),
        status: res.status,
        data: data as { suggestions: MentorshipSuggestedMentorRow[]; totalSuggestions: number },
      };
    }
    const suggestions = (data.suggestions as MentorshipSuggestedMentorRow[]) || [];
    const totalSuggestions =
      typeof data.totalSuggestions === 'number' ? data.totalSuggestions : suggestions.length;
    return { ok: true, data: { suggestions, totalSuggestions } };
  } catch (err) {
    return { ok: false, error: (err as Error)?.message || 'Network error' };
  }
}

/** POST /mentorship/mentee/request — nudge a mentor (PENDING_MENTOR); they still confirm. */
export async function postMentorshipMenteeRequest(
  mentorUserId: string,
): Promise<ApiResult<{ request?: Record<string, unknown> }>> {
  try {
    const headers = await getAuthHeaders();
    ...
    if (!res.ok) {
      ...
      return {
        ...
        status: res.status,
        data: data as { request?: Record<string, unknown> },
      };
    }
    return { ok: true, data: data as { request?: Record<string, unknown> } };
  } catch (err) {
    return { ok: false, error: (err as Error)?.message || 'Network error' };
  }
}
