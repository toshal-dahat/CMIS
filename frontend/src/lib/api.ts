...
export interface MentorshipMentorSuggestion {
  mentorUserId?: string;
  mentorName?: string;
  mentorEmail?: string;
...
> {
  try {
    ...
    const mb = data.mentorBoard as Record<string, unknown> | undefined;
    const mentorBoard: MentorshipMentorBoardInfo | undefined = mb
      ? {
          tier: mb.tier as string | undefined,
          multiplier: typeof mb.multiplier === 'number' ? mb.multiplier : undefined,
          reason: mb.reason as string | undefined,
        }
      : undefined;
...
export async function fetchMentorshipSuggestedMentors(): Promise<
  ApiResult<{ suggestions: MentorshipMentorSuggestion[]; totalSuggestions: number }>
> {
  try {
    const headers = await getAuthHeaders();
    const res = await fetch(`${EXTERNAL_API_BASE}/mentorship/suggested-mentors`, { headers });
    const data = (await res.json().catch(() => ({}))) as Record<string, unknown>;
    if (!res.ok) {
      return {
        ok: false,
        error: (data.error as string) || (data.message as string) || res.statusText || 'Failed to load mentor suggestions',
        status: res.status,
        data: data as unknown as {
          suggestions: MentorshipMentorSuggestion[];
          totalSuggestions: number;
        },
      };
    }
    return { ok: true, data: data as { suggestions: MentorshipMentorSuggestion[]; totalSuggestions: number } };
  } catch (err) {
    return { ok: false, error: (err as Error)?.message || 'Network error' };
  }
}

export async function fetchMentorshipMenteeMatches(): Promise<ApiResult<{ matches: MentorshipMatchRow[]; count: number }>> {
  try {
    const headers = await getAuthHeaders();
...
export async function requestMentorshipFromMentor(mentorUserId: string): Promise<ApiResult<{ request?: MentorshipMatchRow }>> {
  try {
    const headers = await getAuthHeaders();
    const res = await fetch(`${EXTERNAL_API_BASE}/mentorship/mentee/request`, {
      method: 'POST',
    ...
    if (!res.ok) {
      return {
        ok: false,
        error: (data.error as string) || (data.message as string) || res.statusText || 'Could not send request',
        status: res.status,
        data: data as { request?: MentorshipMatchRow },
      };
    }
    return { ok: true, data: data as { request?: MentorshipMatchRow } };
  } catch (err) {
    return { ok: false, error: (err as Error)?.message || 'Network error' };
  }
}
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ...
        status: res.status,
        data: data as { accepted?: MentorshipMatchRow; detail?: string },
      };
    }
