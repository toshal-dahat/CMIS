...
export async function fetchMentorshipMenteeMatches(): Promise<ApiResult<{ matches: MentorshipMatchRow[]; count: number }>> {
  try {
    ...
    const raw = (data.matches as MentorshipMatchRow[]) || [];
    const matches = raw.map((row) => {
      const parsed = parseMentorBoardPayload((row as unknown as Record<string, unknown>).mentorBoard);
      return parsed ? { ...row, mentorBoard: parsed } : row;
    });
...
export async function fetchMentorshipMatches(): Promise<
  ApiResult<{ matches: MentorshipMatchRow[]; count: number; mentorBoard?: MentorshipMentorBoardInfo }>
> {
  try {
    const headers = await getAuthHeaders();
    ...
    if (!res.ok) {
      return {
        ...
        status: res.status,
        data: data as { matches: MentorshipMatchRow[]; count: number; mentorBoard?: MentorshipMentorBoardInfo },
      };
    }
    const matches = (data.matches as MentorshipMatchRow[]) || [];
    const count = typeof data.count === 'number' ? data.count : matches.length;
    const mentorBoard = parseMentorBoardPayload(data.mentorBoard);
    return { ok: true, data: { matches, count, mentorBoard } };
  } catch (err) {
    return { ok: false, error: (err as Error)?.message || 'Network error' };
  }
}
