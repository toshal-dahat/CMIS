<script lang="ts">
  ...
  import { verifyPartnerEmailForOtp } from './partnerDomain';
  ...
  let partnerVerifying = $state(false);
  ...
  async function submitEmail() {
    ...
    // TAMU: unchanged (Google). Non-TAMU: partner domain must exist before OTP.
    if (normalized && !normalized.endsWith('@tamu.edu')) {
      partnerVerifying = true;
      authStatus = 'Verifying company email…';
      const verify = await verifyPartnerEmailForOtp(email);
      partnerVerifying = false;
      if (!verify.ok) {
        authStatus = '';
...
{#if showAuthPanel}
  <div class="auth-overlay" role="dialog" aria-modal="true" aria-label="Sign in">
    <div class="auth-card">
      ...
      {#if !otpStep}
        <p class="auth-hint">
          TAMU users (<span class="auth-hint-mono">@tamu.edu</span>) are redirected to Google. Everyone else: use your
          registered partner company email; we verify your domain before sending an OTP.
        </p>
        <input type="email" bind:value={email} placeholder="you@example.com" disabled={partnerVerifying} />
        <button type="button" class="btn-signin" onclick={submitEmail} disabled={partnerVerifying}>
          {partnerVerifying ? 'Verifying…' : 'Continue'}
        </button>
      {:else}
...
<main class="main">
  ...
  <section class="features">
    ...
    <div class="features-grid">
      {#if !($authUser && (signedInProfileRole === 'friend' || signedInProfileRole === 'investor' || signedInProfileRole === 'admin'))}
      ...
      {/if}
      {#if !($authUser && signedInProfileRole === 'admin')}
