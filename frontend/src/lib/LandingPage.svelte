<script lang="ts">
  ...
  async function completePostSignIn(user: AuthUser) {
    ...
    if (routeToPendingCheckin()) return;
    setViewIfActive('landing');
    await maybeShowGraduationPrompt();
  }

  function routeToPendingCheckin(): boolean {
    ...
    try {
      ...
      eventCheckinContext.set({
        ...
        prefilledScanText: pending.token,
      });
      window.sessionStorage.removeItem('cmis:pendingCheckin');
      setViewIfActive('event-checkin-scanner');
      return true;
    } catch {
      return false;
    }
...
<main class="main">
  ...
  <section class="features">
    ...
    <div class="features-grid">
      {#if $authUser && $profile.mentorshipInterested === true}
      ...
      {#if signedInProfileRole === 'admin'}
        ...
        <article class="feature-card" role="button" tabindex="0" onkeydown={(e) => e.key === 'Enter' && currentView.set('student-engagement-analytics')} onclick={() => currentView.set('student-engagement-analytics')}>
          ...
          <p class="feature-desc">Track RSVP attendance trends, top attendees, and major/year heatmap insights.</p>
        </article>
