          ...
          <div style="display: flex; gap: 0.6rem; align-items: center; flex-wrap: wrap;">
            ...
            {#if $currentRole === "admins"}
              <button class="btn-create" onclick={() => {
                ...
                let defaultOverrides: Record<string, number> = {};
                if ($velvetRopeStore?.tiers) {
                  $velvetRopeStore.tiers.forEach(t => defaultOverrides[t.tierId] = t.earlyAccessHours);
                }

                form = { title: "", date: "", category: "", capacity: 50, description: "", location: "", velvetRopeBypass: false, tierOverrides: defaultOverrides };
                showCreateForm = true;
              }}>+ Create Event</button>
            {/if}
          </div>
              ...
              <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 0.75rem; margin-bottom: 0.5rem;">
                {#each $velvetRopeStore.tiers as tier}
                  <label style="font-size: 0.85rem; font-weight: 500;">
                    ...
                    <input type="number" bind:value={form.tierOverrides[tier.tierId]} min="0" style="padding: 0.35rem; margin-top: 0.25rem; width: 100%; box-sizing: border-box;" />
                  </label>
                {/each}
              </div>
            {/if}
