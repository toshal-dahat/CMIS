<script lang="ts">
  /**
   * EventsDashboard — primary event browsing and RSVP interface (Team 12th Man).
   *
   * Responsibilities:
   *   - Lists and client-side filters events by category, date, and past/future toggle.
   *   - Manages the current user's RSVP state for the session.
   *   - Enforces Velvet Rope tier-based early access via computeVelvetRopeStatus.
   *   - Admin controls: create, edit, soft-cancel events; view attendees; generate QR codes.
   *
   * RSVP state machine:
   *   userRsvpIds   Set<eventId> of events the user has any engagement with (CONFIRMED or
   *                 WAITLISTED). Checked first (O(1)) to decide whether to show the cancel
   *                 button vs the RSVP button.
   *
   *   userRsvpsMap  Map<eventId, RsvpRecord> with full records including `status`. Consulted to
   *                 differentiate "Leave Waitlist" from "Cancel RSVP" button labels, and to
   *                 decide whether to apply an optimistic currentRsvps change on cancel.
   *
   *   handleRsvp()       — Adds eventId to both structures. If the server returns WAITLISTED the
   *                        optimistic currentRsvps increment is skipped (the seat was not granted).
   *
   *   handleCancelRsvp() — Checks isWaitlisted from the map before deleting the entry, then
   *                        removes from both structures. Only decrements currentRsvps when the
   *                        cancelled entry was CONFIRMED; WAITLISTED users never held a seat.
   *
   * Reactivity notes (Svelte 5 runes):
   *   Map and Set mutations do not trigger $state reactivity on their own. Each mutation is
   *   followed by a self-assignment (e.g. `userRsvpsMap = userRsvpsMap`) to force a reference
   *   change and queue a rerender. filteredEvents uses $derived.by because the filter logic
   *   spans multiple conditions that cannot be expressed as a single reactive expression.
   */
  ...
  /** RSVP timestamp or N/A for walk-in check-ins (backend stores `NA` when there was no RSVP). */
  function formatRsvpDetailLabel(rsvpAt?: string): string {
    const raw = (rsvpAt ?? "").trim();
    if (!raw || raw === "NA" || raw === "N/A") return "N/A";
  ...
  let filteredEvents = $derived.by(() => {
    ...
    // Time filter
    if (!showPastEvents) {
      // Treat all events on today's calendar date as current/upcoming.
      list = list.filter((e) => new Date(e.date).getTime() >= startOfToday);
    } else {
      list = list.filter((e) => new Date(e.date).getTime() < startOfToday);
    }
  ...
  let formDateOnly = "";
  let formTimeOnly = "";
  let formDeadlineDateOnly = "";
  let formDeadlineTimeOnly = "";
  ...
  function openEditForm(evt: EventItem) {
    ...
    if (evt.rsvpDeadline) {
      const d = new Date(evt.rsvpDeadline);
      const yyyy = d.getFullYear();
      const mm = String(d.getMonth() + 1).padStart(2, '0');
      const dd = String(d.getDate()).padStart(2, '0');
      formDeadlineDateOnly = `${yyyy}-${mm}-${dd}`;
      const hh = String(d.getHours()).padStart(2, '0');
      const min = String(d.getMinutes()).padStart(2, '0');
      formDeadlineTimeOnly = `${hh}:${min}`;
    } else {
      formDeadlineDateOnly = "";
      formDeadlineTimeOnly = "";
    }
    ...
    form = {
      ...
      velvetRopeBypass: evt.velvetRopeBypass || false,
      tierOverrides: evt.tierOverrides ? { ...evt.tierOverrides } : {},
    };
    showCreateForm = true;
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (formDeadlineDateOnly && formDeadlineTimeOnly) {
          form.rsvpDeadline = new Date(`${formDeadlineDateOnly}T${formDeadlineTimeOnly}`).toISOString();
      } else {
          form.rsvpDeadline = undefined;
      }
      
      const cleanedOverrides: Record<string, number> = {};
      if (form.tierOverrides) {
        for (const [k, v] of Object.entries(form.tierOverrides)) {
          if (v !== '' && v !== null && v !== undefined && !Number.isNaN(Number(v))) {
            cleanedOverrides[k] = Number(v);
          }
        }
      }
      form.tierOverrides = cleanedOverrides;
  ...
  async function handleRsvp(eventId: string) {
    ...
    try {
      ...
      userRsvpsMap = userRsvpsMap; // trigger Svelte reactivity — Map mutations don't signal $state

      if (newStatus === "WAITLISTED") {
        actionMsg = result.message || "Event is full. You've been added to the waitlist.";
        // Do NOT increment currentRsvps: a waitlisted user hasn't been granted a seat yet.
      } else {
        actionMsg = "RSVP confirmed!";
        // Optimistically increment without a round-trip; loadData() will reconcile on the next full refresh.
        events = events.map((e) =>
  ...
  async function handleCancelRsvp(eventId: string) {
    ...
    try {
      ...
      // Read status BEFORE deleting — we need it to decide whether to decrement the count.
      const isWaitlisted = userRsvpsMap.get(eventId)?.status?.toUpperCase() === "WAITLISTED";
      userRsvpsMap.delete(eventId);
      userRsvpsMap = userRsvpsMap; // trigger Svelte reactivity — Map mutations don't signal $state

      if (!isWaitlisted) {
        // Only a CONFIRMED user held a real seat; waitlisted users never incremented the count.
        events = events.map((e) =>
          e.eventId === eventId
      ...
      <div class="dashboard-header">
        <div class="header-row">
          ...
          <div style="display: flex; gap: 0.6rem; align-items: center; flex-wrap: wrap;">
            {#if userRsvpIds.size > 0}
              <button
                class="btn-calendar-export"
                onclick={async () => {
                  try {
                    const icsText = await getUserScheduleIcs();
              ...
              >Export My Schedule</button>
            {/if}
            {#if $currentRole === "admins"}
              <button class="btn-create" onclick={() => {
                editingEventId = null;
                
                form = { title: "", date: "", category: "", capacity: 50, description: "", location: "", velvetRopeBypass: false, tierOverrides: {} };
                showCreateForm = true;
              }}>+ Create Event</button>
            {/if}
          </div>
        ...
        <div class="modal-card">
          ...
          <div style="display: flex; gap: 1rem;">
            <label style="flex:1; font-size: 0.85rem;"
              >RSVP Deadline Date (Optional)<input type="date" bind:value={formDeadlineDateOnly} /></label
            >
            <label style="flex:1; font-size: 0.85rem;"
              >RSVP Deadline Time (Optional)<input type="time" bind:value={formDeadlineTimeOnly} /></label
            >
          </div>
          <label
            >Category
          ...
          <div class="tier-overrides-section">
            <h4 style="margin: 1rem 0 0.5rem 0; font-size: 0.95rem; font-weight: 600; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.25rem; color: var(--maroon);">Tier Overrides (Early Access Hours)</h4>
            {#if $velvetRopeStore?.tiers && form.tierOverrides}
              <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 0.75rem; margin-bottom: 0.5rem;">
                {#each $velvetRopeStore.tiers as tier}
                  <label style="font-size: 0.85rem; font-weight: 500;">
                    {tier.name || tier.tierId.toUpperCase()}
                    <input type="number" bind:value={form.tierOverrides[tier.tierId]} placeholder={`Default: ${tier.earlyAccessHours}h`} min="0" style="padding: 0.35rem; margin-top: 0.25rem; width: 100%; box-sizing: border-box;" />
                  </label>
                {/each}
              </div>
            {/if}
          ...
          <label style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem; cursor: pointer; background: #fff0f0; padding: 0.75rem; border-radius: 4px; border: 1px solid #ffcaca;">
            <input type="checkbox" bind:checked={form.velvetRopeBypass} style="width: auto; margin: 0;" />
            <span style="font-weight: 600; font-size: 0.9rem; color: #d00000;">Bypass Early Access Limits (Open immediately to everyone)</span>
          </label>
            ...
            <ul style="max-height: 300px; overflow-y: auto; text-align: left; padding: 0 0.5rem; margin: 0; list-style: none;">
              {#each attendeesList as user}
                <li style="margin-bottom: 0.75rem; border-bottom: 1px solid var(--border); padding-bottom: 0.75rem;">
                  <strong>{user.userEmail && user.userEmail !== "NA" && user.userEmail !== "N/A" ? user.userEmail : user.userId}</strong> <br/>
                  <small style="color: var(--text-muted);">RSVP'd at: {formatRsvpDetailLabel(user.rsvpAt)}</small>
                </li>
              {/each}
            </ul>
        ...
        {#each filteredEvents as evt (evt.eventId)}
          <div class="event-card" class:canceled={evt.status === 'CANCELED'}>
            ...
            <div class="card-actions">
              {#if $authUser}
                {@const velvetStatus = computeVelvetRopeStatus(evt.date, evt.createdBy || "", ($authUser as any)?.userId || ($authUser as any)?.username || "", $velvetRopeStore, evt.velvetRopeBypass, evt.tierOverrides, evt.rsvpDeadline)}
                {#if velvetStatus.countdownMessage && !userRsvpIds.has(evt.eventId) && evt.currentRsvps < evt.capacity && evt.status !== 'CANCELED' && !velvetStatus.isRsvpClosed}
                ...
                {:else if userRsvpIds.has(evt.eventId)}
                  ...
                  {@const isConfirmed = userRsvpsMap.get(evt.eventId)?.status?.toUpperCase() !== "WAITLISTED"}
                  {#if !isCheckedIn}
                    <button
                      class="btn-cancel-rsvp"
                      onclick={() => handleCancelRsvp(evt.eventId)}
                    >{isConfirmed ? "Cancel RSVP" : "Leave Waitlist"}</button>
                  {/if}
                  {#if isConfirmed}
                    <a
                      href={getEventCalendarUrl(evt.eventId)}
                      download="event.ics"
                      class="btn-calendar"
                  ...
                  {/if}
                {:else if velvetStatus.isRsvpClosed}
                  <button class="btn-full" disabled style="opacity: 0.6;">RSVP Closed</button>
