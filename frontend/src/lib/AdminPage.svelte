<script lang="ts">
  ...
  import { getTiersFromConfig, getThemeFromConfig } from '../admin-api/config';
  import { getCompanies, createCompany, updateCompany, deleteCompany } from '../admin-api/companies';
  import { createTier, updateTier, deleteTier } from '../admin-api/tiers';
  ...
  async function loadAll() {
    try {
      loading = true;
      error = null;
      // Companies come directly from companies API
      // Tiers and theme come from config API
      [companies, tiers, theme] = await Promise.all([
        getCompanies(),
        getTiersFromConfig(),
        getThemeFromConfig()
      ]);
  ...
  // ── Cache wait helper ─────────────────────────────────────────────────────
  async function waitForCacheAndReload(step: string) {
    cacheMsg = step;
    cacheLoader.startCountdown();
  ...
  async function handleSaveTier() {
    ...
    try {
      ...
      await waitForCacheAndReload('Waiting for config cache to refresh...');
    } catch (err: any) {
      cacheLoading = false;
      cacheMsg = '';
      error = err.message;
...
<CacheLoader
  bind:this={cacheLoader}
  show={cacheLoading}
  title="Updating Configuration"
  stepMsg={cacheMsg}
>
  <svelte:fragment slot="note">
    Config cache takes ~1 minute to propagate across the ecosystem.
  </svelte:fragment>
</CacheLoader>
