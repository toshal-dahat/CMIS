...
export async function getUploadUrl(
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}${STUDENT_API_PREFIX}/resumes/upload-url`, {
      method: 'POST',
      headers,
...
export async function completeUpload(resumeId: string): Promise<CompleteUploadResult> {
  try {
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_BASE}${STUDENT_API_PREFIX}/resumes/complete`, {
      method: 'POST',
      headers,
...
export async function listMyResumes(): Promise<ListMyResumesResult> {
  try {
    const headers = await getAuthHeaders();
    const res = await fetch(`${API_BASE}${STUDENT_API_PREFIX}/resumes/me`, {
      headers,
      cache: 'no-store',
    });
...
export async function getDownloadUrl(resumeId: string): Promise<GetDownloadUrlResult> {
  try {
    ...
    const res = await fetch(
      `${API_BASE}${STUDENT_API_PREFIX}/resumes/${encodeURIComponent(resumeId)}/download-url`,
      { headers }
    );
    const data = (await res.json().catch(() => ({}))) as Record<string, unknown>;
    if (!res.ok) {
