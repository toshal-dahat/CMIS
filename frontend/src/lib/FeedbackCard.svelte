<script lang="ts">
  ...
  async function loadData(released: boolean, refresh = false) {
    if (!released) {
      loading = false;
      return;
    }
    try {
      ...
      error = "";
      const synthesis = await getSynthesizedFeedback(competitionId, teamId, refresh);
    ...
    <div id="printable-feedback" class="unlocked-content scale-in printable-area">
      ...
      <div class="card-footer no-print">
        ...
        <div class="action-group">
          <button class="refresh-btn" disabled={refreshing} onclick={handleRefresh}>
...
<style>
  ...
  .refresh-btn:hover { background: #50000008; border-color: #500000; }
