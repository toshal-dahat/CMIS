<script lang="ts">
  ...
  let criteria = $derived(
    selectedCompetition?.rubric?.length ? selectedCompetition.rubric : DEFAULT_RUBRIC
  );
  ...
  async function handleGenerateSummary() {
    ...
    summary = null;
    try {
      summary = await generateSummary(selectedCompetitionId, scoringTeam.teamId);
    } catch (e: any) {
  ...
  async function handleSubmitScore() {    if (!scoringTeam || !selectedCompetitionId) return;
    scoreLoading = true;
    scoreError = "";
    try {
...
<div class="judge-page">
  <div class="judge-inner">
    <button type="button" class="back-link" onclick={goBack}>
      <span class="back-arrow">&larr;</span> {selectedCompetitionId && assignments.length > 1 ? "Back to competitions" : "Back to home"}
    </button>
      ...
      {#if !selectedCompetitionId}
        <div class="competition-grid">
          {#each assignments as a}
            ...
            <button class="competition-card" onclick={() => selectCompetition(a.competitionId)}>
              <div class="card-top">
              ...
              <div class="card-footer">
                <span class="deadline">📅 Deadline: {fmtDate(comp?.submissionDeadline ?? null)}</span>
      ...
      <div class="summary-panel">
        <div class="summary-header">
          ...
          <button
            ...
            title={!scoringTeam.hasSubmission ? "No submission available to summarize" : "Generate AI summary of submission"}
          >
            {#if summaryLoading}
        ...
        {#if !scoringTeam.hasSubmission}
          <p class="summary-hint">No submission uploaded yet — summary unavailable.</p>
        ...
        {:else}
          <p class="summary-hint">Click "Generate Summary" to get an AI-powered overview of this team's submission.</p>
        {/if}
      </div>
        ...
        {#each criteria as criterion (criterion.key)}
          <div class="criterion-row">
            ...
            <div class="rating-input">
              ...
              />
              <span class="rating-value">{scoreRatings[criterion.key] ?? (criterion.min ?? 1)}</span>
            </div>
          </div>
        {/each}
      </div>
...
<style>
  ...
  .score-na { color: var(--text-muted, #888); }
  ...
  .summary-body p { margin: 0.5rem 0 0; }
