<script lang="ts">
  ...
  // AI Summary
  let summary: SummaryResult | null = $state(null);
...
<div class="judge-page">
  <div class="judge-inner">
    ...
    {:else}
      ...
      <!-- ── Competition Cards (shown if multiple assignments) ── -->
      {#if assignments.length > 1}
        <div class="competition-cards-section">
          <p class="cards-label">Select a Competition</p>
          <div class="competition-cards-grid">
            {#each assignments as a}
              {@const comp = competitions.get(a.competitionId)}
              <button
                type="button"
                class="comp-card {selectedCompetitionId === a.competitionId
                  ? 'comp-card--selected'
                  : ''}"
                onclick={() => selectCompetition(a.competitionId)}
              >
                {#if selectedCompetitionId === a.competitionId}
                  <span class="comp-card-check">✓</span>
                {/if}
                <h3 class="comp-card-name">{comp?.name || a.competitionId}</h3>
                {#if comp?.description}
                  <p class="comp-card-desc">{comp.description}</p>
                {/if}
                <div class="comp-card-footer">
                  <span class="comp-card-deadline">
                    📅 {fmtDate(comp?.submissionDeadline ?? null)}
                  </span>
                  {#if selectedCompetitionId === a.competitionId}
                    <span class="comp-card-active-badge">Active</span>
                  {/if}
                </div>
              </button>
            {/each}
      ...
      <!-- Competition Info Card (only shown for single assignment or after card selection) -->
  ...
  <div class="modal-overlay" role="dialog" aria-modal="true">
    <div class="modal-card">
      ...
      <div class="summary-panel">
        <div class="summary-header">
          ...
          <button
            ...
            disabled={summaryLoading || !scoringTeam.hasSubmission}
            title={!scoringTeam.hasSubmission
              ? "No submission available to summarize"
              : "Generate AI summary of submission"}
          >
            {#if summaryLoading}
        ...
        {#if !scoringTeam.hasSubmission}
          <p class="summary-hint">
            No submission uploaded yet — summary unavailable.
          </p>
        ...
        {:else if summary}
          <div class="summary-body">
            {#if summary.cached}
              <span class="summary-cached-badge">Cached</span>
            {/if}
            <p>{summary.summary}</p>
          </div>
        {:else}
          ...
          <div class="criterion-row">
            ...
            <div class="rating-input">
              ...
              <span class="rating-value"
                >{scoreRatings[criterion.key] ?? criterion.min ?? 1}</span
              >
            </div>
          </div>
        {/each}
      ...
      <div class="modal-actions">
        <button class="btn-cancel" onclick={() => (scoringTeam = null)}
          >Cancel</button
        >
        <button
          class="btn-submit"
...
<style>
  ...
  .comp-card--selected {
    border-color: var(--maroon);
    background: linear-gradient(135deg, #fff 60%, rgba(80, 0, 0, 0.04) 100%);
    box-shadow:
      0 0 0 3px rgba(80, 0, 0, 0.12),
      0 4px 16px rgba(80, 0, 0, 0.08);
  }
  .comp-card-check {
    position: absolute;
    top: 0.75rem;
  ...
  .summary-cached-badge {
    ...
    font-size: 0.75rem;
    font-weight: 600;
    background: var(--gold-light, rgba(198, 140, 83, 0.15));
    color: var(--maroon, #500000);
    border-radius: 20px;
  }
  .spinner-sm {
