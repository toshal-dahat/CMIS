<script lang="ts">
  ...
  import {
    ...
    getMentorshipAdminLiveSnapshot,
    getLatestMentorshipMatchingRun,
    getMentorshipMatchingSchedule,
    listMentorshipMatchingRuns,
    postMentorshipAdminRecycleExpired,
    postMentorshipAdminPairSuggestByEmail,
  ...
  let runProgressExpectedSeconds = 120;
  let runProgressModeLabel = 'Fast sync';
  let runProgressHeartbeat = 0;
  let busyDryRun = false;
  let busyDiagnostics = false;
  let busyRepair = false;
  let busyRecycleExpired = false;
  let busyPairSuggestByEmail = false;
  ...
  let mentorEmailForPairSuggest = '';
  let menteeEmailForPairSuggest = '';
  let runProgressTimer: ReturnType<typeof setInterval> | null = null;

  let latestRun: MentorshipMatchingRunDetail | null = null;
  let liveSnapshot: { pairs?: Array<Record<string, unknown>>; mentors?: Array<Record<string, unknown>>; pairCount?: number } | null =
    null;
  ...
  function asPairs(): Array<Record<string, unknown>> {
    const raw = liveSnapshot?.pairs ?? latestRun?.snapshot?.pairs;
  ...
  function latestSnapshotSummary(): { pairCount: number; mentorCount: number } {
    if (liveSnapshot) {
      const livePairs = Array.isArray(liveSnapshot.pairs) ? liveSnapshot.pairs.length : Number(liveSnapshot.pairCount || 0);
      const liveMentors = Array.isArray(liveSnapshot.mentors) ? liveSnapshot.mentors.length : 0;
      if (livePairs > 0 || liveMentors > 0) {
        return { pairCount: livePairs, mentorCount: liveMentors };
      }
    }
    const run = latestRun;
  ...
  function runProgressPercent(): number {
    void runProgressHeartbeat;
    if (!runProgressStartedAt || runProgressExpectedSeconds <= 0) return 0;
    const elapsedSec = Math.max(0, Math.floor((Date.now() - runProgressStartedAt) / 1000));
    const pct = Math.round((elapsedSec / runProgressExpectedSeconds) * 100);
    // Cap at 95% until server confirms completion, then instantly finish.
    return Math.min(95, Math.max(3, pct));
  }

  function runProgressRemainingLabel(): string {
    void runProgressHeartbeat;
    if (!runProgressStartedAt || runProgressExpectedSeconds <= 0) return '';
    const elapsedSec = Math.max(0, Math.floor((Date.now() - runProgressStartedAt) / 1000));
    const remaining = Math.max(0, runProgressExpectedSeconds - elapsedSec);
  ...
  async function startRunProgressWatcher(
    ...
    const tick = async () => {
      ...
      try {
        ...
        const stText = String(latest.status || 'UNKNOWN').trim();
        runProgressMessage =
          phase && phase !== stText ? `Latest status: ${stText} · ${phase}` : `Latest status: ${stText}`;
        ...
        const live = await getMentorshipAdminLiveSnapshot().catch(() => ({ snapshot: undefined }));
        liveSnapshot = live.snapshot ?? liveSnapshot;
  ...
  async function refreshAll(): Promise<void> {
    ...
    error = null;
    try {
      const [latest, history, sched, live] = await Promise.all([
        ...
        getMentorshipAdminLiveSnapshot().catch(() => ({ snapshot: undefined })),
      ]);
      latestRun = latest.run ?? null;
      runHistory = history.runs ?? [];
      schedule = { ...schedule, ...sched };
      liveSnapshot = live.snapshot ?? null;
      await ensureLatestRunHasSnapshotPairs();
  ...
  async function runNow(resetMatches: boolean, runBatchMatching = true): Promise<void> {
    ...
    try {
      const run = await startMentorshipMatchingRun({
        resetMatches,
        runBatchMatching,
        // Use live snapshot endpoint for operator table; skipping heavy inline snapshot
        // keeps reset+sync jobs noticeably faster.
        includeSnapshot: false,
      });
      const isQueued = String(run.status || '').toUpperCase() === 'QUEUED' || run.accepted === true;
      const queuedRunId = String(run.runId || '').trim();
      runProgressExpectedSeconds = resetMatches ? 300 : 120;
      runProgressModeLabel = resetMatches ? 'Reset + sync' : 'Fast sync';
      runProgressHeartbeat = 0;
      if (isQueued) {
  ...
  async function runDryPreview(): Promise<void> {
    if (busyDryRun || busyRun) return;
    busyDryRun = true;
    error = null;
    success = null;
    dryRunSummary = '';
    try {
      const preview = await startMentorshipMatchingRun({
        dryRun: true,
        dryRunPerMentorLimit: 1,
      });
      const pairCount = Number(preview.pairCount || (Array.isArray(preview.pairs) ? preview.pairs.length : 0));
      const errCount = Array.isArray(preview.errors) ? preview.errors.length : 0;
      dryRunSummary =
        `Dry-run preview generated ${pairCount} proposed pair(s)` +
        `${errCount > 0 ? ` with ${errCount} warning(s)` : ''}. No database rows were changed.`;
      success = dryRunSummary;
    } catch (e: any) {
      error = e?.message || 'Failed to run dry-run preview.';
    } finally {
      busyDryRun = false;
    }
  }
  ...
  async function runMenteeCapDiagnostics(): Promise<void> {
    ...
    try {
      ...
      const ttlHours = Number(payload.suggestionTtlHours || 0);
      const expiredQueue = Number(payload.expiredSuggestionRows || 0);
      const activeQueue = Number(payload.activeSuggestionRows || 0);
      diagnosticsSummary =
        `Diagnostics loaded: ${payload.returned ?? diagnosticsRows.length} row(s), ` +
        `${payload.driftCount ?? 0} drifted. ` +
        `Suggestion TTL: ${ttlHours > 0 ? `${ttlHours}h` : 'off'} ` +
        `(active queue: ${activeQueue}, expired: ${expiredQueue}).`;
  ...
  async function recycleExpiredSuggestions(): Promise<void> {
    busyRecycleExpired = true;
    error = null;
    success = null;
    try {
      const payload = await postMentorshipAdminRecycleExpired({
        limit: diagnosticsLimit,
        autoReassign: true,
      });
      const processed = Number(payload.processed || 0);
      const reassigned = Number(payload.reassignedRows || 0);
      const ttl = Number(payload.ttlHours || 0);
      if (payload.disabled) {
        success = 'Suggestion TTL is disabled. Set MENTORSHIP_SUGGESTION_TTL_HOURS > 0 to recycle expired queue rows.';
      } else {
        success =
          `Expired suggestion recycle completed: ${processed} row(s) closed, ${reassigned} reassign attempt(s) started` +
          `${ttl > 0 ? ` (TTL ${ttl}h).` : '.'}`;
      }
      await runMenteeCapDiagnostics();
      await refreshAll();
    } catch (e: any) {
      error = e?.message || 'Failed to recycle expired suggestions.';
    } finally {
      busyRecycleExpired = false;
    }
  }

  async function runPairSuggestByEmail(): Promise<void> {
    const mentorEmail = mentorEmailForPairSuggest.trim().toLowerCase();
    const menteeEmail = menteeEmailForPairSuggest.trim().toLowerCase();
    if (!mentorEmail || !menteeEmail) {
      error = 'Mentor email and mentee email are required.';
      success = null;
      return;
    }
    busyPairSuggestByEmail = true;
    error = null;
    success = null;
    try {
      const payload = await postMentorshipAdminPairSuggestByEmail({ mentorEmail, menteeEmail });
      const deletedRows = Number(payload?.reset?.deletedRows || 0);
      const resolvedMentor = String(payload?.resolved?.mentorUserId || '—');
      const resolvedMentee = String(payload?.resolved?.menteeUserId || '—');
      success =
        `Recovery pairing queued. Cleared ${deletedRows} mentee row(s), then created a fresh suggestion. ` +
        `mentor=${resolvedMentor}, mentee=${resolvedMentee}.`;
      await refreshAll();
    } catch (e: any) {
      error = e?.message || 'Failed to reset mentee and suggest by email.';
    } finally {
      busyPairSuggestByEmail = false;
    }
  }
</script>
...
<section class="section">
  ...
  {#if runProgressActive}
    <div class="run-progress" role="status" aria-live="polite">
      ...
      <div class="run-progress-subline">
        <span>{runProgressModeLabel}</span>
        <span>{runProgressRemainingLabel()}</span>
      </div>
      <div class="run-progress-track" aria-hidden="true">
        <div class="run-progress-fill" style={`width: ${runProgressPercent()}%`}></div>
      </div>
      <div class="run-progress-percent">{runProgressPercent()}%</div>
    ...
    <div class="cards-grid">
      <div class="card">
        ...
        <div class="card-title-row">
          ...
          <span class="mode-badge">Fast mode ON</span>
        </div>
        <div class="card-content">
          <p class="hint">
            Manually trigger a fast mentorship sync now (does not hard-reset existing match rows).
          </p>
          <p class="run-mode-meta"><strong>Run now</strong> expected runtime: <strong>~1-2 min</strong></p>
          <p class="run-mode-meta"><strong>Reset + sync</strong> expected runtime: <strong>~2-5 min</strong></p>
          <p class="hint compact">
            Use <strong>Run now</strong> for regular allocation updates. Use <strong>Reset + sync</strong> only for
            stale/inconsistent state recovery.
          </p>
          <div class="card-actions">
            <button class="btn-add" onclick={() => void runNow(false, true)} disabled={busyRun}>
              {busyRun ? 'Running…' : 'Run now'}
            </button>
            <button class="btn-secondary" onclick={() => void runDryPreview()} disabled={busyRun || busyDryRun}>
              {busyDryRun ? 'Previewing…' : 'Dry-run preview'}
            </button>
            <button
              class="btn-delete"
              onclick={() => {
                if (
                  confirm(
                    'Reset all mentorship rows and immediately rebuild matching now? This clears mentor/mentee state first, then re-syncs suggestions.',
                  )
                ) {
                  void runNow(true, true);
                }
              }}
              disabled={busyRun}>Reset + sync</button>
          </div>
          {#if dryRunSummary}
            <div class="diagnostics-summary">{dryRunSummary}</div>
          {/if}
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-icon">🧯</div>
        <div class="card-title">Recovery pair (by email)</div>
        <div class="card-content form-grid">
          <p class="hint">
            Hard-reset one mentee's matching rows and assign a fresh suggestion to a specific mentor in one step.
          </p>
          <label>Mentor email
            <input
              type="email"
              bind:value={mentorEmailForPairSuggest}
          ...
          <label>Mentee email
            <input
              type="email"
              bind:value={menteeEmailForPairSuggest}
              placeholder="mentee@domain.com"
              autocomplete="off"
            />
          </label>
          <button
            class="btn-add"
            onclick={() => void runPairSuggestByEmail()}
            disabled={busyPairSuggestByEmail || busyRun || loading}
          >
            {busyPairSuggestByEmail ? 'Applying…' : 'Reset mentee + suggest to mentor'}
          </button>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-content form-grid">
          ...
          <div class="card-actions">
            <button
              class="btn-add"
              onclick={() => void runMenteeCapDiagnostics()}
              disabled={busyDiagnostics || busyRepair || busyRecycleExpired}
            >
            ...
            <button
              class="btn-secondary"
              onclick={() => void recycleExpiredSuggestions()}
              disabled={busyDiagnostics || busyRepair || busyRecycleExpired}
            >
              {busyRecycleExpired ? 'Recycling…' : 'Recycle expired'}
            </button>
          </div>
...
<style>
  ...
  .mode-badge {
    display: inline-flex;
    align-items: center;
    border: 1px solid color-mix(in srgb, var(--maroon) 30%, white);
    color: var(--maroon);
