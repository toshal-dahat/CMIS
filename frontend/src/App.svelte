...
import MentorshipPage from "./lib/MentorshipPage.svelte";
  ...
  import MentorshipOperatorPage from "./lib/MentorshipOperatorPage.svelte";
  ...
  import EventSuccessAnalyticsPage from "./lib/EventSuccessAnalyticsPage.svelte";
  import SurveyResponsePage from "./lib/SurveyResponsePage.svelte";
  import TeamSubmissionPage from "./lib/TeamSubmissionPage.svelte";
  import AdminCompetitionFeedbackPage from "./lib/AdminCompetitionFeedbackPage.svelte";
  import { getTheme } from "./admin-api/theme";
  import { currentRole } from "./lib/stores/roleStore";
  ...
  onMount(() => {
    void (async () => {
      const pendingSurveyEventId = new URLSearchParams(window.location.search || "").get("surveyEventId");
      const pendingSurveyUserId = new URLSearchParams(window.location.search || "").get("surveyUserId");
      const pendingSurveyUserEmail = new URLSearchParams(window.location.search || "").get("surveyUserEmail") || "";
      if (pendingSurveyEventId && pendingSurveyUserId) {
        try {
          window.sessionStorage.setItem(
            "cmis:pendingSurvey",
            JSON.stringify({ eventId: pendingSurveyEventId, userId: pendingSurveyUserId, userEmail: pendingSurveyUserEmail }),
          );
        } catch {
        ...
        currentView.set("event-survey-respond");
        return;
      }
...
{:else if $currentView === "events"}
  <!-- Team 12th Man: event catalog, RSVP, waitlist, Velvet Rope tier gating, and calendar export -->
  <div class="full-page-wrap">
    <EventsDashboard />
  </div>
...
{:else if $currentView === "mentorship-operator"}
  <div class="full-page-wrap">
    <MentorshipOperatorPage />
  </div>
{:else if $currentView === "case-competitions"}
  <div class="full-page-wrap">
    {#if $currentRole === "admins"}
      <AdminCompetitionFeedbackPage />
    {:else}
      <TeamSubmissionPage />
    {/if}
  </div>
...
{:else if $currentView === "event-success-analytics"}
  <div class="full-page-wrap">
    <EventSuccessAnalyticsPage />
  </div>
{:else if $currentView === "event-survey-respond"}
  <div class="full-page-wrap">
    <SurveyResponsePage />
  </div>
{:else}
