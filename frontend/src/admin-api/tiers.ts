...
// GET all tiers directly from tiers API
export async function getTiers(): Promise<Tier[]> {
  try {
    ...
    // API returns plain array, not { tiers: [] }
    const tiers: Tier[] = Array.isArray(data) ? data : (data.tiers ?? []);
    return tiers.sort((a: Tier, b: Tier) => (a.rank || 0) - (b.rank || 0));
  } catch (error) {
